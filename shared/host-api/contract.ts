...
import type { ChatOrganizationSnapshot } from '../types/chat-organization';
...
export type WindowShowMenuPayload = { menu: WindowMenuId; x: number; y: number };
...
export type GatewayConfigurePayload = {
  url: string;
  authMode: 'token' | 'password';
  credential: string;
};
export type GatewayConfigureResult = HostSuccess & {
  status?: GatewayStatus;
};

export type ChatOrganizationCreateProjectPayload = { name: string; path: string };
export type ChatOrganizationCreateFolderPayload = {
  projectId: string;
  parentId?: string | null;
  name: string;
};
export type ChatOrganizationRenamePayload = {
  kind: 'project' | 'folder';
  id: string;
  name: string;
};
export type ChatOrganizationDeletePayload = { id: string; confirmed?: boolean };
export type ChatOrganizationMoveChatPayload = {
  chatKey: string;
  projectId: string;
  folderId?: string | null;
  beforeChatKey?: string | null;
};
export type ChatOrganizationMoveFolderPayload = {
  folderId: string;
  projectId: string;
  parentId?: string | null;
};
export type ChatOrganizationUnplaceChatPayload = { chatKey: string; workspacePath: string };
export type ChatOrganizationPinPayload = { chatKey: string; pinned: boolean; beforeChatKey?: string | null };
export type ChatOrganizationPinNodePayload = {
  kind: 'project' | 'folder';
  id: string;
  pinned: boolean;
};
export type ChatOrganizationCollapsePayload = {
  kind: 'project' | 'folder' | 'pinned';
  id?: string;
  collapsed: boolean;
};
...
export type AgentPersonalityResult = { content: string; missing?: boolean } & OptionalHostSuccess;
export type AgentUpdatePersonalityPayload = { id: string; content: string };
...
export type ProviderUsageWindow = {
  label: string;
  usedPercent: number;
  period?: 'five_hour' | 'weekly' | 'monthly';
  resetAt?: number;
  accountId?: string;
  accountName?: string;
...
export type ProviderUsageSnapshot = {
  ...
  summary?: string;
  billing?: Array<
    | { type: 'budget'; used: number; limit: number; unit: string; period?: string }
    | { type: 'balance'; amount: number; unit: string }
  >;
  error?: string;
};

export type ProviderLimitsPayload = {
  providerKey?: string;
  modelRef?: string;
  forceRefresh?: boolean;
};

export type ProviderLimitsResult = {
  updatedAt: number;
  providers: ProviderUsageSnapshot[];
};

export type OmniRouteLimitsConfig = {
  baseUrl: string;
  configured: boolean;
};

export type OmniRouteLimitsConfigurePayload = {
  baseUrl: string;
  managementToken?: string;
  clearToken?: boolean;
};

export type OmniRouteLimitsConfigureResult = HostSuccess & OmniRouteLimitsConfig;
...
export type HostApiContract = {
  ...
  shell: {
    ...
    openTerminal: (payload: ShellPathPayload) => void;
  };
  webBrowser: {
  ...
  window: {
    ...
    showMenu: (payload: WindowShowMenuPayload) => void;
    minimize: () => void;
    maximize: () => void;
    close: () => void;
    isMaximized: () => boolean;
  ...
  gateway: {
    ...
    configure: (payload: GatewayConfigurePayload) => GatewayConfigureResult;
    status: () => GatewayStatus;
    start: () => HostSuccess;
    stop: () => HostSuccess;
    restart: () => HostSuccess;
  ...
  agents: {
    ...
    personality: (payload: AgentIdPayload) => AgentPersonalityResult;
    updatePersonality: (payload: AgentUpdatePersonalityPayload) => AgentPersonalityResult;
  ...
  chatOrganization: {
    ...
    createProject: (payload: ChatOrganizationCreateProjectPayload) => ChatOrganizationSnapshot;
    createFolder: (payload: ChatOrganizationCreateFolderPayload) => ChatOrganizationSnapshot;
    rename: (payload: ChatOrganizationRenamePayload) => ChatOrganizationSnapshot;
    deleteProject: (payload: ChatOrganizationDeletePayload) => ChatOrganizationSnapshot;
    deleteFolder: (payload: ChatOrganizationDeletePayload) => ChatOrganizationSnapshot;
    moveChat: (payload: ChatOrganizationMoveChatPayload) => ChatOrganizationSnapshot;
    moveFolder: (payload: ChatOrganizationMoveFolderPayload) => ChatOrganizationSnapshot;
    unplaceChat: (payload: ChatOrganizationUnplaceChatPayload) => ChatOrganizationSnapshot;
    pinChat: (payload: ChatOrganizationPinPayload) => ChatOrganizationSnapshot;
    pinNode: (payload: ChatOrganizationPinNodePayload) => ChatOrganizationSnapshot;
    setCollapsed: (payload: ChatOrganizationCollapsePayload) => ChatOrganizationSnapshot;
  };
  chat: {
  ...
  usage: {
    ...
    providerLimits: (payload?: ProviderLimitsPayload) => ProviderLimitsResult;
    omniRouteConfig: () => OmniRouteLimitsConfig;
    configureOmniRoute: (payload: OmniRouteLimitsConfigurePayload) => OmniRouteLimitsConfigureResult;
  };
};
