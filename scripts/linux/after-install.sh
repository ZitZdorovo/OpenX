...
# Post-installation script for OpenX on Linux
...
# Create symbolic link for OpenX app binary
if [ -x /opt/OpenX/openx ]; then
    ln -sf /opt/OpenX/openx /usr/local/bin/openx 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/OpenX/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/OpenX/chrome-sandbox' || true
else
    chmod 0755 '/opt/OpenX/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/OpenX/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/openx'
...
echo "OpenX has been installed successfully."
