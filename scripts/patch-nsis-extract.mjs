...
const PATCH_MARKER = 'OpenX-patched-v2: extract directly to $INSTDIR and fail closed';
const LEGACY_PATCH_MARKER = 'OpenX-patched: extract directly to $INSTDIR';
const LEGACY_CONTINUE_ON_EXTRACT_FAILURE = 'continuing overwrite install anyway';
const FATAL_EXTRACT_FAILURE_DETAIL = 'Failed to extract OpenX files after multiple attempts.';
const ROLLBACK_EXTRACT_FAILURE_DETAIL = 'Restoring previous OpenX installation after failed update';

const PATCHED_EXTRACT_MACRO = [
  '!macro extractUsing7za FILE',
  `  ; ${PATCH_MARKER}.`,
  '  StrCpy $R9 0',
  '  openx_extract_attempt:',
  '    IntOp $R9 $R9 + 1',
  '    DetailPrint "Extracting OpenX application files (attempt $R9, please wait)..."',
  '    SetOutPath $INSTDIR',
  '    ClearErrors',
  '    Nsis7z::Extract "${FILE}"',
  '    IfErrors 0 openx_extract_done',
  '    ${if} $R9 < 5',
  ...
  '      Pop $1',
  '      Sleep 3000',
  '      Goto openx_extract_attempt',
  '    ${endIf}',
  '    DetailPrint "Failed to extract OpenX files after multiple attempts."',
  '    ${if} $openxRollbackDir != ""',
  '      IfFileExists "$openxRollbackDir\\" 0 openx_extract_show_error',
  '      DetailPrint "Restoring previous OpenX installation after failed update..."',
  '      SetOutPath $TEMP',
  '      RMDir /r "$INSTDIR"',
  '      Rename "$openxRollbackDir" "$INSTDIR"',
  '    ${endIf}',
  '  openx_extract_show_error:',
...
export function patchNsisExtractTemplate(targetPath = EXTRACT_APP_PACKAGE_NSH) {
  ...
  if (hasStaleExtractPatch(original)) {
    console.warn('[patch-nsis-extract] Stale OpenX extract patch detected; replacing with fail-closed patch.');
