; OpenX Custom NSIS Installer/Uninstaller Script
;
...
Function OpenXMoveLegacyInstallDir
  Exch $R6

  ${if} $R6 == ""
    Goto _openx_legacy_move_done
  ${endIf}
  ${if} $R6 == $INSTDIR
    Goto _openx_legacy_move_done
  ${endIf}

  IfFileExists "$R6\" 0 _openx_legacy_move_done
    DetailPrint "Moving previous OpenX installation at $R6 out of the way..."
    SetOutPath $TEMP
  ...
  _openx_legacy_find_free_stale:
    IfFileExists "$R6._stale_$R8\" 0 _openx_legacy_found_free_stale
    IntOp $R8 $R8 + 1
    Goto _openx_legacy_find_free_stale

  _openx_legacy_found_free_stale:
    ClearErrors
    Rename "$R6" "$R6._stale_$R8"
    IfErrors 0 _openx_legacy_stale_moved
      ...
      Rename "$R6" "$R6._stale_$R8"
      IfErrors 0 _openx_legacy_stale_moved
      DetailPrint "Removing previous OpenX installation at $R6..."
      nsExec::ExecToStack 'cmd.exe /c rd /s /q "$R6"'
      Pop $0
      Pop $1
      Goto _openx_legacy_move_done
  _openx_legacy_stale_moved:
    ExecShell "" "cmd.exe" `/c ping -n 61 127.0.0.1 >nul & rd /s /q "$R6._stale_$R8"` SW_HIDE

  _openx_legacy_move_done:
    ClearErrors
    Pop $R6
FunctionEnd

!macro openxMoveLegacyInstallDir ROOT_KEY
  ReadRegStr $R6 ${ROOT_KEY} "${INSTALL_REGISTRY_KEY}" InstallLocation
  Push $R6
  Call OpenXMoveLegacyInstallDir
!macroend
!endif
...
!macro customCheckAppRunning
  ...
  DetailPrint "Extracting OpenX runtime files. This can take a few minutes on slower disks or while antivirus scanning is active."
  ...
  ${if} $R0 == 0
    ...
    # This covers OpenX.exe (multiple Electron processes), openclaw-gateway.exe,
  ...
  ; Even if OpenX.exe was not detected as running, orphan child processes
  ...
  ; PowerShell path-based kill may miss processes if the old OpenX was installed
  ...
  ; commands instead of nsProcess here: field diagnostics showed OpenX.exe can
  ...
  _openx_verify_closed:
    ...
    ${if} $R0 == 0
      ...
      Sleep 2000
      ${if} $R7 < 5
        Goto _openx_verify_closed
      ${endIf}
      MessageBox MB_RETRYCANCEL|MB_ICONEXCLAMATION "OpenX is still running and cannot be replaced safely. Please close OpenX and retry installation." /SD IDCANCEL IDRETRY _openx_verify_closed
      SetErrorLevel 2
      Quit
    ${endIf}

  !ifndef BUILD_UNINSTALLER
    StrCpy $openxRollbackDir ""
    ...
    !insertmacro openxMoveLegacyInstallDir HKCU
    !insertmacro openxMoveLegacyInstallDir HKLM
  ...
  ; "Can't modify OpenX's files" retry loop -> "OpenX 无法关闭" dialog.
  ;
  ...
  _found_free_stale:
    ...
    IfErrors 0 _stale_moved
      ...
      IfFileExists "$INSTDIR\" 0 _recreate_clean_instdir
        ...
        MessageBox MB_OK|MB_ICONEXCLAMATION "Unable to replace the previous OpenX installation because files are still locked. Please close OpenX and retry installation." /SD IDOK
        SetErrorLevel 2
        Quit
      ...
      Goto _instdir_clean
  _stale_moved:
    StrCpy $openxRollbackDir "$INSTDIR._stale_$R8"
    CreateDirectory "$INSTDIR"
  _instdir_clean:
...
; "OpenX 无法关闭" retry dialog when the old uninstaller fails.
;
...
!macro customInstall
  ...
  DetailPrint "Clearing stale OpenX registry entries from the opposite install scope..."
  ${if} $installMode == "all"
  ...
  ; Wait 60s before starting deletion to avoid I/O contention with OpenX's
...
!macro customUnInstall
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "Do you want to remove OpenX application data?$\r$\n$\r$\nThis will delete the OpenX local and roaming app data.$\r$\nLegacy OpenX app data is preserved for migration.$\r$\n$\r$\nYour .openclaw folder (configuration & skills) will be preserved.$\r$\nSelect 'No' to keep all data for future reinstallation." \
    /SD IDNO IDYES _cu_removeData IDNO _cu_skipRemove

  _cu_removeData:
    ; Kill any lingering OpenX processes (and their child process trees) to
    ...
    RMDir /r "$LOCALAPPDATA\openx"
    RMDir /r "$APPDATA\openx"

    ; --- Retry: if directories still exist (locked files), wait and try again ---

    ; Check AppData\Local\openx
    IfFileExists "$LOCALAPPDATA\openx\*.*" 0 _cu_localDone
      Sleep 3000
      RMDir /r "$LOCALAPPDATA\openx"
      IfFileExists "$LOCALAPPDATA\openx\*.*" 0 _cu_localDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$LOCALAPPDATA\openx"'
        Pop $0
        Pop $1
    _cu_localDone:

    ; Check AppData\Roaming\openx
    IfFileExists "$APPDATA\openx\*.*" 0 _cu_roamingDone
      Sleep 3000
      RMDir /r "$APPDATA\openx"
      IfFileExists "$APPDATA\openx\*.*" 0 _cu_roamingDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$APPDATA\openx"'
        Pop $0
        Pop $1
    _cu_roamingDone:
    ...
    StrCpy $R3 ""
    IfFileExists "$LOCALAPPDATA\openx\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $LOCALAPPDATA\openx"
    IfFileExists "$APPDATA\openx\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $APPDATA\openx"
  ...
  _cu_enumLoop:
    ...
    RMDir /r "$R3\AppData\Local\openx"
    RMDir /r "$R3\AppData\Roaming\openx"
