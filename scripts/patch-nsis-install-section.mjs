 ...
 * replacement in that inner process, so the old OpenX.exe can remain alive,
...
const PATCH_MARKER = 'OpenX-patched-v2: run app-running guard in assisted UAC inner instance';
...
const PATCHED_ASSISTED_CHECK = [
  ...
  '  ; UAC inner instance; skipping CHECK_APP_RUNNING there can leave OpenX.exe',
