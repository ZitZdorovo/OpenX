...
function resolveManifestPath(): string {
  if (app.isPackaged) {
    return join(process.resourcesPath, 'openx-extensions.json');
  }
  return join(app.getAppPath(), 'openx-extensions.json');
}
...
export async function loadExtensionsFromManifest(): Promise<void> {
  ...
  if (existsSync(manifestPath)) {
    ...
    logger.debug('[extensions] No openx-extensions.json found, loading all builtin extensions');
  }
