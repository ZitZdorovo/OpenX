...
async function loadFullCronRunReplies(
  ...
  await Promise.all(runs.map(async (entry) => {
    ...
    const transcriptResult = await gatewayManager.rpc<{ messages?: RawMessage[] }>('chat.history', {
...
async function readSessionStoreEntry(
  ...
  const result = await gatewayManager.rpc<{ sessions?: Array<Record<string, unknown>> }>('sessions.list', {
    limit: 500,
  });
  return result.sessions?.find((entry) => entry.key === sessionKey || entry.sessionKey === sessionKey);
}
...
async function listCronJobs(gatewayManager: GatewayManager): Promise<CronJob[]> {
  ...
  const jobs = data?.jobs ?? (Array.isArray(result) ? result as GatewayCronJob[] : []);
