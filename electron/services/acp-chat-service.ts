...
import { getGatewayCredential } from './secrets/gateway-credential-store';
...
export class AcpChatService {
  ...
  async sendPrompt(payload: AcpChatPromptPayload): Promise<AcpChatOperationResult> {
    ...
    try {
      ...
      await connection.prompt({
        ...
        // OpenX correlation metadata in the protocol extension envelope.
  ...
  private async initializeConnection(): Promise<AcpConnection> {
    if (!this.gateway?.isConnected()) throw new Error('Remote Gateway is not connected');
    return this.initializeConnectionOnce(1);
  }
  ...
  private async spawnConnection(): Promise<ClientSideConnection> {
    const [settings, credential] = await Promise.all([getAllSettings(), getGatewayCredential()]);
    if (!credential) throw new Error('Gateway credential is not configured');
    const spec = getOpenClawEmbeddedForkSpec(['acp']);
    spec.options.env = {
      ...spec.options.env,
      OPENCLAW_GATEWAY_URL: settings.gatewayUrl,
      ...(credential.mode === 'token'
        ? { OPENCLAW_GATEWAY_TOKEN: credential.secret, OPENCLAW_GATEWAY_PASSWORD: undefined }
        : { OPENCLAW_GATEWAY_PASSWORD: credential.secret, OPENCLAW_GATEWAY_TOKEN: undefined }),
    };
  ...
  private async buildPromptBlocks(payload: AcpChatPromptPayload): Promise<ContentBlock[]> {
    ...
    if (media.length > 0) {
      ...
      for (const item of media) {
        ...
        const itemStat = await fsP.stat(item.filePath);
        if (itemStat.isDirectory()) {
          blocks.push({
            ...
            _meta: { openx: { stagingId: item.stagingId } },
          });
          continue;
        }
        if (mimeType.startsWith('image/')) {
          ...
          const blob = await fsP.readFile(item.filePath, 'base64');
          blocks.push({
            type: 'resource',
            resource: {
              uri: `openx-attachment:///${encodeURIComponent(item.fileName ?? item.stagingId)}`,
              blob,
              mimeType: item.mimeType,
            },
            _meta: {
