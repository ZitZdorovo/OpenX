...
export function resolveOpenXStagingDir(stateDir = resolveOpenClawStateDir()): string {
  return join(resolve(stateDir), 'media', 'outbound', 'openx-staging');
}
...
function parseOutgoingUrl(uri: string): { attachmentId: string; sessionKey: string } | null {
  let url: URL;
  try {
    url = uri.startsWith('/') ? new URL(uri, 'http://openx.local') : new URL(uri);
  } catch {
    return null;
  }
...
export function createAttachmentAccess(dependencies: AttachmentAccessDependencies): AttachmentAccess {
  ...
  const resolveTarget = async (
    ...
    if (context.localAccess === false && !ref.stagingId) {
      throw new AttachmentFailure('unavailable');
    }
