    ...
    saveImageGenerationSettings: async (payload) => {
      ...
      const normalizeRelayModel = (value: unknown): string => {
        const raw = typeof value === 'string' && value.trim()
          ? value.trim()
          : (current.openAiRelay.model || OPENX_OPENAI_IMAGE_DEFAULT_MODEL);
        const slash = raw.indexOf('/');
        return (slash > 0 ? raw.slice(slash + 1) : raw).trim() || OPENX_OPENAI_IMAGE_DEFAULT_MODEL;
      };
      ...
      if (body.openAiRelayEnabled === true) {
        nextPrimary = `${OPENX_OPENAI_IMAGE_PROVIDER_KEY}/${relayModel}`;
