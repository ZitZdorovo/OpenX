...
function buildOpenXUvConfigToml(): string {
  return [
    '# This file is managed by OpenX.',
    '# It lets uv use OpenX-selected mirrors even when host exec filters UV_INDEX_URL.',
...
export function getOpenXUvConfigFilePath(): string {
  return path.join(getOpenClawConfigDir(), 'openx', 'uv.toml');
}

function ensureOpenXUvConfigFile(): string | null {
  const filePath = getOpenXUvConfigFilePath();
  const content = buildOpenXUvConfigToml();
  ...
  try {
    ...
    logger.warn('Failed to write OpenX uv config file:', err);
    return null;
  }
}
...
export async function getUvMirrorEnv(): Promise<Record<string, string>> {
  ...
  const uvConfigFile = ensureOpenXUvConfigFile();
  return uvConfigFile
