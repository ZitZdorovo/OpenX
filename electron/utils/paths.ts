...
function getElectronApp() {
  ...
  const fallbackUserData = process.env.OPENX_USER_DATA_DIR?.trim() || join(homedir(), '.openx');
...
/**
 * Get OpenX config directory
 */
export function getOpenXConfigDir(): string {
  return join(homedir(), '.openx');
}

/**
 * Get OpenX logs directory
 */
