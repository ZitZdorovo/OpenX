/**
 * Persist OpenX-managed plugin install records into OpenClaw's SQLite
 ...
 * OpenX-authored records win over stale SQLite entries for the same plugin id.
 */
    ...
    if (row) {
      ...
      db.prepare(`
        INSERT INTO installed_plugin_index (
          ...
          ?, 1, 'openx-managed', 'openx-managed',
          1, 'openx-managed', ?, 'source-changed',
          ?, '[]', '[]', ?, ?
        )
      `).run(
...
/**
 * Remove install records that must remain OpenX-managed rather than updated
 * from their raw upstream npm package. Also clean the legacy root-level DB
 * previously written by OpenX before OpenClaw 2026.7.1 moved state to state/.
 */
