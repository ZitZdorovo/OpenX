...
import {
  OPENX_OPENAI_IMAGE_DEFAULT_MODEL,
  OPENX_OPENAI_IMAGE_PROVIDER_KEY,
 ...
 *
 * This intentionally reads auth-profiles.json rather than OpenX's provider
...
/**
 * Backfill `reserveTokensFloor` on compaction configs that OpenX or OpenClaw
 ...
 * nor `contextTokens`. Rows written by older OpenX versions only carried
 ...
 * OpenX-written entries historically only included `{ id, name }`.
 *
 ...
 * The bundled OpenClaw distribution OpenX ships does not register that harness,
 ...
 * (written by earlier OpenX builds that did not pin the runtime) get
 * repaired before the next Gateway reload picks them up. Without this, users
 * who upgrade OpenX while still pointing at an OpenAI provider would keep
...
/**
 * Configure a OpenX-owned OpenAI-compatible image provider.
...
export async function syncOpenAiCompatibleImageRelay(params: {
  ...
  await mutateOpenClawConfig((config) => {
    if (!params.enabled) {
      const models = (config.models || {}) as Record<string, unknown>;
      const providers = (models.providers || {}) as Record<string, unknown>;
      if (providers[OPENX_OPENAI_IMAGE_PROVIDER_KEY]) {
        delete providers[OPENX_OPENAI_IMAGE_PROVIDER_KEY];
        models.providers = providers;
        config.models = models;
      }
      ...
      const primary = typeof imageGenerationModel?.primary === 'string'
        ...
        : '';
      if (defaults && imageGenerationModel && primary.startsWith(`${OPENX_OPENAI_IMAGE_PROVIDER_KEY}/`)) {
        const remainingFallbacks = Array.isArray(imageGenerationModel.fallbacks)
          ? imageGenerationModel.fallbacks.filter((fallback): fallback is string => (
            typeof fallback === 'string'
              && !fallback.trim().toLowerCase().startsWith(`${OPENX_OPENAI_IMAGE_PROVIDER_KEY}/`)
          ))
          : [];
      ...
      removePluginRegistrations(config, [OPENX_OPENAI_IMAGE_PROVIDER_KEY]);
      normalizeAgentsDefaultsCompactionMode(config);
      return;
    }
    ...
    if (modelIds.length === 0) {
      modelIds.push(OPENX_OPENAI_IMAGE_DEFAULT_MODEL);
    }
    const existingModels = readModelsProvider(config, OPENX_OPENAI_IMAGE_PROVIDER_KEY)?.models;
    ...
    upsertOpenClawProviderEntry(config, OPENX_OPENAI_IMAGE_PROVIDER_KEY, {
      baseUrl,
    ...
    const relayProvider = readModelsProvider(config, OPENX_OPENAI_IMAGE_PROVIDER_KEY);
    ...
    ensurePluginRegistrationEnabled(config, OPENX_OPENAI_IMAGE_PROVIDER_KEY);
    normalizeAgentsDefaultsCompactionMode(config);
  });

  if (!params.enabled) {
    await removeProviderKeyFromOpenClaw(OPENX_OPENAI_IMAGE_PROVIDER_KEY);
  }
  if (params.apiKey?.trim()) {
    await saveProviderKeyToOpenClaw(OPENX_OPENAI_IMAGE_PROVIDER_KEY, params.apiKey.trim());
  }
}
...
export function readOpenAiCompatibleImageRelayState(
  ...
  const openxRelay = readModelsProvider(config, OPENX_OPENAI_IMAGE_PROVIDER_KEY);
  const relayBaseUrl = typeof openxRelay?.baseUrl === 'string' ? openxRelay.baseUrl.trim() : '';
  if (relayBaseUrl) {
    return { enabled: true, baseUrl: relayBaseUrl, providerKey: OPENX_OPENAI_IMAGE_PROVIDER_KEY };
  }

  // Backward compatibility for OpenX builds that used models.providers.openai
  // for image relay. New saves move to the OpenX-owned provider above.
 ...
 * Used by OpenX to seed the provider store when it's empty but providers are
...
/**
 * Write the OpenX gateway token into ~/.openclaw/openclaw.json.
 */
export async function syncGatewayTokenToConfig(token: string): Promise<void> {
  const gatewayPort = Number(new URL(await getSetting('gatewayUrl')).port) || 18789;
  await mutateOpenClawConfig((config) => {
    const gateway = (
 ...
 * which means conversations disappear after roughly one day.  OpenX sets
...
export async function batchSyncConfigFields(token: string): Promise<void> {
  ...
  const changed = await mutateOpenClawConfig((config) => {
    ...
    // Migrate OpenX's exact legacy disabled default once, and otherwise seed
 ...
 * OpenClaw/OpenX versions or external tools.
 *
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  ...
  await mutateOpenClawConfig(async (config) => {
    ...
    // for OpenX to properly integrate with its updated tool system.
    ...
    // OpenX keeps direct skill-creator authoring instead, so deny the workshop
    ...
    if (denyResult.modified) {
      ...
      toolsModified = true;
      console.log('[sanitize] Added "skill_workshop" to tools.deny for OpenX desktop');
    } else if (!Array.isArray(toolsConfig.deny) || toolsConfig.deny.length !== denyResult.deny.length) {
      toolsConfig.deny = denyResult.deny;
      toolsModified = true;
    }

    // OpenX uses the managed browser and web_fetch for explicit navigation,
    ...
    if (webSearchDenyResult.modified) {
      ...
      toolsModified = true;
      console.log('[sanitize] Added "web_search" to tools.deny for OpenX desktop');
    } else if (
    ...
    // OpenX is a local desktop app where the user is the trusted operator.
    ...
    if (execConfig.security !== 'full' || execConfig.ask !== 'off') {
      ...
      toolsModified = true;
      console.log('[sanitize] Set tools.exec.security="full" and tools.exec.ask="off" to disable exec approvals for OpenX desktop');
    }
    ...
    // agent:main:main), which makes OpenX sidebar conflate feishu, dingtalk,
    ...
    if (sessionConfig.dmScope !== 'per-channel-peer' && sessionConfig.dmScope !== 'per-account-channel-peer') {
      ...
      modified = true;
      console.log('[sanitize] Set session.dmScope="per-channel-peer" so channel DMs appear as separate sessions in OpenX');
    }
    ...
    if (gatewayDenyResult.modified) {
      ...
      console.log('[sanitize] Added "skill_workshop" to gateway.tools.deny for OpenX desktop');
    ...
    if (gatewayWebSearchDenyResult.modified) {
      ...
      gatewayModified = true;
      console.log('[sanitize] Added "web_search" to gateway.tools.deny for OpenX desktop');
    } else if (
    ...
    if (autonomous.enabled !== false) {
      ...
      skillsModified = true;
      console.log('[sanitize] Disabled skills.workshop.autonomous for OpenX desktop');
    }
    ...
    if (skillCreatorEntry.enabled !== true) {
      ...
      skillsModified = true;
      console.log('[sanitize] Enabled bundled skill-creator for direct skill authoring in OpenX desktop');
    }
    ...
    if (typeof plugins === 'object' && !Array.isArray(plugins)) {
      ...
      // OpenX bundles Feishu via the official @larksuite/openclaw-lark
      ...
      // OpenX-critical bundled plugins, active provider plugins, and explicitly
