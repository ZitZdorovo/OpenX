...
const OPENX_BEGIN = '<!-- openx:begin -->';
const OPENX_END = '<!-- openx:end -->';
...
export function buildDefaultOpenXIdentityContent(): string {
  return [
    '# IDENTITY.md - OpenX',
    '',
    '- **Name:** OpenX',
    ...
    '- **Avatar:**',
    '',
    'OpenX uses a default desktop identity instead of chat-first bootstrap.',
    '',
  ].join('\n');
}
...
/**
 * Ensure OpenX-managed workspaces have a non-template IDENTITY.md before the
 * Gateway initializes them. Existing custom identities are preserved.
 */
export async function ensureOpenXIdentityFile(
  workspaceDir: string,
  ...
  const defaultIdentity = buildDefaultOpenXIdentityContent();
  ...
  if (await fileExists(bootstrapPath)) {
    try {
      ...
      logger.info(`Removed chat-first bootstrap file from OpenX workspace (${resolvedWorkspaceDir})`);
    } catch {
      logger.warn(`Failed to remove chat-first bootstrap file: ${bootstrapPath}`);
    }
  } else if (wroteIdentity) {
    logger.info(`Seeded default OpenX identity for workspace (${resolvedWorkspaceDir})`);
  }
}

export async function ensureOpenXDefaultIdentity(): Promise<void> {
  const workspaceDirs = await resolveAllWorkspaceDirs();
  for (const { dir: workspaceDir, waitForGatewaySeed } of workspaceDirs) {
    await ensureOpenXIdentityFile(workspaceDir, { createDir: waitForGatewaySeed });
  }
}
...
/**
 * Merge a OpenX context section into an existing file's content.
 * If markers already exist, replaces the section in-place.
 * Otherwise appends it at the end.
 */
export function mergeOpenXSection(existing: string, section: string): string {
  const wrapped = `${OPENX_BEGIN}\n${section.trim()}\n${OPENX_END}`;
  const beginIdx = existing.indexOf(OPENX_BEGIN);
  const endIdx = existing.indexOf(OPENX_END);
  if (beginIdx !== -1 && endIdx !== -1) {
    return existing.slice(0, beginIdx) + wrapped + existing.slice(endIdx + OPENX_END.length);
  }
 ...
 * for OpenX-managed workspaces.  Removes everything from the heading
...
/**
 * Detect and remove bootstrap .md files that contain only OpenX markers
 * with no meaningful OpenClaw content outside them.
 */
export async function repairOpenXOnlyBootstrapFiles(): Promise<void> {
  const workspaceDirs = await resolveAllWorkspaceDirs();
  for (const { dir: workspaceDir } of workspaceDirs) {
    ...
    for (const file of entries) {
      ...
      const beginIdx = content.indexOf(OPENX_BEGIN);
      const endIdx = content.indexOf(OPENX_END);
      if (beginIdx === -1 || endIdx === -1) continue;

      const before = content.slice(0, beginIdx).trim();
      const after = content.slice(endIdx + OPENX_END.length).trim();
      if (before === '' && after === '') {
        try {
          await unlink(filePath);
          logger.info(`Removed OpenX-only bootstrap file for re-seeding: ${file} (${workspaceDir})`);
        } catch {
          logger.warn(`Failed to remove OpenX-only bootstrap file: ${filePath}`);
        }
      }
    }
  }
...
/**
 * Merge OpenX context snippets into workspace bootstrap files that already
...
type EnsureOpenXContextOptions = {
  /**
...
async function mergeOpenXContextOnce(options: EnsureOpenXContextOptions = {}): Promise<MergeResult> {
  const contextDir = join(getResourcesDir(), 'context');
  if (!(await fileExists(contextDir))) {
    logger.debug('OpenX context directory not found, skipping context merge');
    return { missing: 0, retryableMissing: 0 };
  }

  let files: string[];
  try {
    files = (await readdir(contextDir)).filter((f) => f.endsWith('.openx.md'));
  } catch {
    return { missing: 0, retryableMissing: 0 };
  }
  ...
  for (const { dir: workspaceDir, waitForGatewaySeed } of workspaceDirs) {
    ...
    for (const file of files) {
      const targetName = file.replace('.openx.md', '.md');
      ...
      const merged = mergeOpenXSection(existing, section);
      // Compare against on-disk content so we persist changes even when only
      // First Run stripping happened and the OpenX section stayed identical.
      if (merged !== originalExisting) {
        await writeFile(targetPath, merged, 'utf-8');
        logger.info(`Merged OpenX context into ${targetName} (${workspaceDir})`);
      }
    }
  }
...
const MAX_RETRIES = 5;
let ensureOpenXContextPromise: Promise<void> | null = null;
let ensureOpenXContextWaitsForAll = false;

/**
 * Ensure OpenX context snippets are merged into the openclaw workspace
 * bootstrap files.
 */
export async function ensureOpenXContext(options: EnsureOpenXContextOptions = {}): Promise<void> {
  if (ensureOpenXContextPromise) {
    if (options.waitForAllConfiguredWorkspaces && !ensureOpenXContextWaitsForAll) {
      return ensureOpenXContextPromise.then(() => ensureOpenXContext(options));
    }
    return ensureOpenXContextPromise;
  }

  ensureOpenXContextWaitsForAll = options.waitForAllConfiguredWorkspaces === true;
  ensureOpenXContextPromise = runEnsureOpenXContext(options).finally(() => {
    ensureOpenXContextPromise = null;
    ensureOpenXContextWaitsForAll = false;
  });
  return ensureOpenXContextPromise;
}

async function runEnsureOpenXContext(options: EnsureOpenXContextOptions): Promise<void> {
  let result = await mergeOpenXContextOnce(options);
  if (result.retryableMissing === 0) {
    if (result.missing > 0) {
      logger.debug(`OpenX context merge skipped ${result.missing} non-ready file(s)`);
    }
    return;
  }
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    result = await mergeOpenXContextOnce(options);
    if (result.retryableMissing === 0) {
      logger.info(`OpenX context merge completed after ${attempt} retry(ies)`);
      return;
    }
    logger.debug(`OpenX context merge: ${result.retryableMissing} startup file(s) still missing (retry ${attempt}/${MAX_RETRIES})`);
  }

  logger.warn(`OpenX context merge: ${result.retryableMissing} startup file(s) still missing after ${MAX_RETRIES} retries`);
}
