...
import { ensureOpenXIdentityFile } from './openclaw-workspace';
...
export async function listExistingAgentIdsOnDisk(): Promise<Set<string>> {
...
export async function provisionAgentFilesystem(
  ...
  // on first use, but OpenX still pre-seeds IDENTITY.md so desktop workspaces
  ...
  await ensureOpenXIdentityFile(targetWorkspace, { createDir: true });
...
export async function listAgentsSnapshot(): Promise<AgentsSnapshot> {
  ...
  return buildSnapshotFromConfig(config as AgentConfigDocument);
}
...
export async function createAgent(
  name: string,
  _options?: { inheritWorkspace?: boolean },
  ...
  logger.info('Created remote agent config entry', { agentId: createdAgentId });
  return snapshot!;
}
...
export async function deleteAgentConfig(agentId: string): Promise<{ snapshot: AgentsSnapshot; removedEntry: AgentListEntry }> {
  ...
  logger.info('Deleted remote agent config entry', { agentId });
  return result!;
}
