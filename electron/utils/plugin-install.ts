...
export function fixupPluginManifest(targetDir: string): void {
  ...
  try {
    ...
    // permissive schema that preserves OpenX's existing channel config fields.
  ...
  try {
    ...
    // Keep the real upstream npm package name/spec even though OpenX patches
    ...
    // Gateway starts. Restore metadata previously rewritten by older OpenX
    // compatibility code.
...
/**
 * Channel plugins whose OpenX-managed mirrors need synchronized install
...
type TrustedOfficialExtensionPlugin = {
  ...
  pluginId?: string;
  /** Path records keep OpenClaw from replacing a OpenX-patched mirror. */
...
const TRUSTED_OFFICIAL_EXTENSION_PLUGINS: Record<string, TrustedOfficialExtensionPlugin> = {
  ...
  // WeCom intentionally runs under OpenX's legacy-compatible `wecom` id even
  ...
  // managed npm records during its post-core smoke check. Make OpenX's complete
  ...
  'openx-openai-image': {
    npmName: 'openx-openai-image-plugin',
    recordSource: 'path',
  },
};
...
/**
 * Persist a OpenX-mirrored plugin install record in OpenClaw's canonical SQLite
...
export async function syncTrustedOfficialPluginInstallRecord(
  ...
  // Remove aliases left by older OpenX/OpenClaw ownership conventions, but do
...
/**
 * Remove metadata for a OpenX mirror that is no longer configured. This must
...
export function ensureOpenXOpenAiImagePluginInstalled(): Promise<{ installed: boolean; warning?: string }> {
  return ensurePluginInstalled(
    'openx-openai-image',
    buildCandidateSources('openx-openai-image'),
    'OpenX OpenAI Image',
  );
}
...
const ALL_BUNDLED_PLUGINS = [
  ...
  { fn: ensureOpenXOpenAiImagePluginInstalled, label: 'OpenX OpenAI Image' },
] as const;
