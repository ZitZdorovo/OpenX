...
interface SyncProxyOptions {
   ...
   * currently disabled in OpenX settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync OpenX global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  if (syncState.result === 'preserved') {
    logger.info('Skipped Telegram proxy sync because OpenX proxy is disabled and preserve mode is enabled');
