...
import { ensureOpenXOpenAiImagePluginInstalled } from './plugin-install';
import {
  listAgentsSnapshot,
...
export async function setImageGenerationConfig(
  ...
  await mutateOpenClawConfig((config) => {
    ...
    // OpenX image generation is configured as one explicit custom endpoint.
    // Keep OpenClaw from appending other authenticated image providers such as
    // minimax-portal/image-01 after the configured OpenX image provider.
...
function resolveOpenAiImageRelayModelId(
  ...
  if (primary) {
    ...
    if (slash > 0 && slash < primary.length - 1) {
      ...
      if (provider === OPENX_OPENAI_IMAGE_PROVIDER_KEY || provider === 'openai') {
        return primary.slice(slash + 1).trim() || OPENX_OPENAI_IMAGE_DEFAULT_MODEL;
      }
    }
  }
  ...
  const providerEntry = providers && typeof providers === 'object'
    ? (providers as Record<string, unknown>)[OPENX_OPENAI_IMAGE_PROVIDER_KEY]
    : null;
  return extractModelIdFromProviderEntry(providerEntry) ?? OPENX_OPENAI_IMAGE_DEFAULT_MODEL;
}
...
export async function getImageGenerationSettingsSnapshot(): Promise<ImageGenerationSettingsSnapshot> {
  ...
  const relayAuthProvider = relayState.providerKey === 'openai' ? 'openai' : OPENX_OPENAI_IMAGE_PROVIDER_KEY;
...
export async function applyOpenAiImageRelaySettings(params: {
  ...
  if (params.enabled) {
    const plugin = await ensureOpenXOpenAiImagePluginInstalled();
    if (!plugin.installed) {
      throw new Error(plugin.warning || 'Failed to install OpenX OpenAI Image plugin');
    }
  }
  ...
  if (imageModelIds.length === 0) {
    imageModelIds.push(OPENX_OPENAI_IMAGE_DEFAULT_MODEL);
  }
...
export async function listImageGenerationProvidersFromRuntime(): Promise<ImageGenerationProviderRow[]> {
  ...
  return rows.filter((row) => row.id === OPENX_OPENAI_IMAGE_PROVIDER_KEY);
}
