...
- A run that starts and finishes while OpenX is fully closed is unobserved and cannot produce a justified unread marker with OpenClaw 2026.6.10.
...
- Local attention is presentation state, not a Gateway-wide read receipt. Another client opening a conversation does not clear OpenX's local unread bit.
...
6. Remove the local transition store and retire the `openx.session-attention` persistence key through an explicit versioned cleanup so old local bits cannot reappear.
...
Do not begin this migration merely because a type exists in an unbundled upstream branch. The bundled Gateway must expose and persist the complete contract used by OpenX.

## Rejected Alternatives

- ACP prompt/timeline state: scoped to OpenX-owned agent connections and misses channel or other-client runs.
