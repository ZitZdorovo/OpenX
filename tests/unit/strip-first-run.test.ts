...
const { testHome } = vi.hoisted(() => ({
  testHome: `/tmp/openx-openclaw-workspace-${Math.random().toString(36).slice(2)}`,
}));
...
import {
  ensureOpenXContext,
  ensureOpenXDefaultIdentity,
  ensureOpenXIdentityFile,
  mergeOpenXSection,
...
describe('stripFirstRunSection', () => {
  ...
  it('still changes AGENTS content when only First Run is removed', () => {
    ...
    const merged = mergeOpenXSection(stripped, section);

    expect(merged).not.toBe(original);
    expect(merged).not.toContain('## First Run');
    expect(merged).toContain('## Session Startup');
    expect(merged).toContain('<!-- openx:begin -->');
    expect(merged).toContain('<!-- openx:end -->');
  });
});

describe('ensureOpenXIdentityFile', () => {
  it('writes a default OpenX identity when the workspace has none', async () => {
    const workspaceDir = join(testHome, '.openclaw', 'workspace');
    await mkdir(workspaceDir, { recursive: true });

    await ensureOpenXIdentityFile(workspaceDir);

    await expect(readFile(join(workspaceDir, 'IDENTITY.md'), 'utf-8')).resolves.toContain('OpenX');
  });
...
describe('ensureOpenXDefaultIdentity', () => {
  it('creates the default workspace and seeds IDENTITY.md for startup-owned workspaces', async () => {
    await ensureOpenXDefaultIdentity();

    await expect(readFile(join(testHome, '.openclaw', 'workspace', 'IDENTITY.md'), 'utf-8')).resolves.toContain('OpenX');
  });
});

describe('ensureOpenXContext', () => {
  it('does not wait for missing files in non-default agent workspaces', async () => {
    ...
    const result = await Promise.race([
      ensureOpenXContext().then(() => 'done'),
      new Promise((resolve) => setTimeout(() => resolve('timeout'), 200)),
    ]);

    expect(result).toBe('done');
    await expect(readFile(join(defaultWorkspace, 'AGENTS.md'), 'utf-8')).resolves.toContain('## OpenX Environment');
    await expect(readFile(join(defaultWorkspace, 'TOOLS.md'), 'utf-8')).resolves.toContain('## OpenX Tool Notes');
