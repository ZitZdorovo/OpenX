...
vi.mock('@electron/utils/store', () => ({
  getAllSettings: vi.fn(async () => ({ gatewayUrl: 'ws://127.0.0.1:18789' })),
}));

vi.mock('@electron/services/secrets/gateway-credential-store', () => ({
  getGatewayCredential: vi.fn(async () => ({ mode: 'token', secret: 'test-token' })),
}));
...
async function createService(connection = createConnection(), accessRegistry = createPassthroughAccessRegistry()) {
  ...
  const service = new AcpChatService(
    ...
    connection as never,
    createConnectedGateway() as never,
  );
...
describe('AcpChatService', () => {
  ...
  it('rewrites fresh-session ACP updates to the OpenX session key for the renderer', async () => {
  ...
  ])('commits a remote access grant only after $operation resolves', async ({ createIfMissing, operation }) => {
    const parent = mkdtempSync(join(tmpdir(), 'openx-acp-service-access-'));
  ...
  it('restores the previous access grant when a later load fails', async () => {
    const parent = mkdtempSync(join(tmpdir(), 'openx-acp-service-rollback-'));
  ...
  it('rejects a prompt cwd that differs from the registered execution cwd', async () => {
    const workspaceRoot = mkdtempSync(join(tmpdir(), 'openx-acp-service-prompt-cwd-'));
  ...
  it('builds ACP prompt blocks from message and media', async () => {
    const imagePath = join(tmpdir(), `openx-acp-service-${Date.now()}.png`);
    const filePath = join(tmpdir(), `openx-acp-service-${Date.now()}.txt`);
      ...
      expect(connection.prompt).toHaveBeenCalledWith({
        ...
        prompt: [
          { type: 'text', text: 'Inspect attachments' },
          {
            ...
            mimeType: 'image/png',
            uri: imagePath,
            _meta: { openx: { stagingId: 'staged-image', fileName: 'image.png' } },
          },
          {
            type: 'resource',
            _meta: { openx: { stagingId: 'staged-notes', fileName: 'notes.txt' } },
            resource: {
              uri: 'openx-attachment:///notes.txt',
              mimeType: 'text/plain',
              blob: Buffer.from('plain text').toString('base64'),
            },
          },
        ],
