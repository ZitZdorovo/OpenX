...
describe('session-key-utils', () => {
  ...
  it('treats OpenX desktop session keys as non-channel', () => {
  ...
  it('excludes cron and channel keys from desktop-only session keys', () => {
    expect(isOpenXDesktopSessionKey('agent:main:main')).toBe(true);
    expect(isOpenXDesktopSessionKey('agent:main:session-1710000000000')).toBe(true);
    expect(isOpenXDesktopSessionKey('agent:main:feishu:ou_abc123')).toBe(false);
    expect(isOpenXDesktopSessionKey('agent:main:cron:heartbeat')).toBe(false);
  });
  ...
  it('does not treat missing metadata as proof of a hidden heartbeat session', () => {
    const sessions: ChatSession[] = [{ key: 'agent:main:main', displayName: 'OpenX' }];

    expect(findHiddenOpenClawHeartbeatSession('agent:main:main', sessions)).toBeNull();
  });

  it('does not hide a real conversation only because it is titled OpenX', () => {
