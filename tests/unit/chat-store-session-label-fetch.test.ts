...
describe('chat store session label summary hydration', () => {
  ...
  it('hydrates a cwd-only truncated derived title from the session summary', async () => {
    ...
    const workspacePath = '/Users/zhuoxu/workspace/openx-playground';
    gatewayRpcMock.mockImplementation(async (method: string) => {
      if (method === 'sessions.list') {
        return {
          sessions: [
            {
              key: sessionKey,
              displayName: 'ACP',
              derivedTitle: '[Working directory: ~/workspace/openx-playground]…',
  ...
  it('hydrates existing sidebar session titles as soon as sessions load', async () => {
    gatewayRpcMock.mockImplementation(async (method: string) => {
      if (method === 'sessions.list') {
        return {
          sessions: [
            { key: 'agent:main:session-a', displayName: 'OpenX', updatedAt: 1000 },
            { key: 'agent:main:session-b', displayName: 'OpenX', updatedAt: 1001 },
            { key: 'agent:main:main', displayName: 'OpenX', updatedAt: 1002 },
          ],
        };
      }
