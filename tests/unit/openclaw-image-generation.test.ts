...
const { testHome, testUserData } = vi.hoisted(() => {
  ...
  return {
    testHome: `/tmp/openx-openclaw-image-gen-${suffix}`,
    testUserData: `/tmp/openx-openclaw-image-gen-user-data-${suffix}`,
  };
});
...
vi.mock('@electron/utils/plugin-install', () => ({
  ensureOpenXOpenAiImagePluginInstalled: ensureImagePluginInstalledMock,
}));
