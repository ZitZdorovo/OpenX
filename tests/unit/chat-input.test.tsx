...
const { agentsState, chatState, gatewayState, providersState, providerUsageState, artifactPanelMocks } = vi.hoisted(() => ({
  agentsState: {
  ...
  chatState: {
    ...
    sessions: [] as Array<{ key: string; displayName?: string }>,
  },
  gatewayState: {
...
vi.mock('@/stores/provider-usage', () => ({
  useProviderUsageStore: (selector: (state: typeof providerUsageState) => unknown) => selector(providerUsageState),
}));
...
describe('ChatInput agent targeting', () => {
  ...
  it('renders editable workspace selector in the composer footer', () => {
    render(
      <TooltipProvider>
        <ChatInput
          onSend={vi.fn()}
          workspaceLabel="~/workspace/OpenX"
          workspacePath="/Users/alex/workspace/OpenX"
    ...
    expect(button).toHaveTextContent('~/workspace/OpenX');
    expect(button).toHaveAttribute('title', '/Users/alex/workspace/OpenX');
    expect(button).not.toHaveAttribute('aria-disabled', 'true');
  });

  it('aligns request stats with the workspace selector and omits gateway status from the composer footer', () => {
    render(
      <TooltipProvider>
        <ChatInput
          onSend={vi.fn()}
          ...
          workspaceReadOnly={false}
          onSelectWorkspace={vi.fn()}
          usage={{ used: 100, size: 1_000 }}
        />
      </TooltipProvider>,
    );

    const workspaceSelector = screen.getByTestId('chat-workspace-selector');
    const requestStats = screen.getByTestId('chat-request-stats-button');

    expect(workspaceSelector.parentElement?.parentElement?.parentElement).toBe(requestStats.parentElement?.parentElement?.parentElement);
    expect(screen.getByTestId('chat-composer-footer')).toHaveClass('px-[5px]', 'mt-[15px]', 'h-6');
  ...
  it('keeps gateway startup status out of the composer footer', () => {
    gatewayState.status = { state: 'running', port: 18789, gatewayReady: false };
    agentsState.agents = [
      {
        id: 'main',
    ...
    expect(screen.queryByText(/gateway starting/i)).not.toBeInTheDocument();
  });
  ...
  it('shows a full-window attachment target when files enter anywhere in the app', () => {
    renderChatInput();
