...
describe('chat session management', () => {
  ...
  it('keeps workspace identity when selecting and acknowledging an ACP session', async () => {
    ...
    await vi.waitFor(() => expect(sessionRenameMock).toHaveBeenCalledWith(key, 'Investigate issue'));
    expect(useChatStore.getState().sessions.find((session) => session.key === key)?.label).toBe('Investigate issue');
  });

  it('turns the first prompt into a concise topic and syncs it to OpenClaw', async () => {
    const key = 'agent:main:session-topic';
    const { useChatStore } = await import('@/stores/chat');
    useChatStore.setState({
      currentSessionKey: key,
      currentAgentId: 'main',
    ...
    useChatStore.getState().acknowledgeAcpSessionCreated(
      key,
      '/workspace',
      'Так, смотри, пожалуйста, сделай удобное меню проектов. Остальное обсудим позже.',
    );

    await vi.waitFor(() => expect(sessionRenameMock).toHaveBeenCalledWith(
      key,
      'удобное меню проектов.',
    ));
    expect(useChatStore.getState().sessionLabels[key]).toBe('удобное меню проектов.');
  });
  ...
  it('cleans persisted attention after a successful hard delete', async () => {
    ...
    expect(window.localStorage.getItem('openx.session-attention')).not.toContain(deletedKey);
  });
