...
describe('env-path', () => {
  ...
  it('prepends entry with Windows delimiter', () => {
    setPlatform('win32');
    const next = prependPathEntry({ Path: 'C:\\Windows\\System32' }, 'D:\\openx\\resources\\bin');
    expect(next.path).toBe('D:\\openx\\resources\\bin;C:\\Windows\\System32');
    expect(next.env.Path).toBe('D:\\openx\\resources\\bin;C:\\Windows\\System32');
  });

  it('prepends entry with POSIX delimiter', () => {
    setPlatform('linux');
    const next = prependPathEntry({ PATH: '/usr/bin:/bin' }, '/opt/openx/bin');
    expect(next.path).toBe('/opt/openx/bin:/usr/bin:/bin');
    expect(next.env.PATH).toBe('/opt/openx/bin:/usr/bin:/bin');
  });
});
