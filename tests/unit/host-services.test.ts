...
const {
  ...
  testOpenClawConfigDir: '/tmp/openx-host-services-openclaw',
...
vi.mock('@electron/utils/logger', async (importOriginal) => {
  ...
  return {
    logger: {
      ...
      warn: vi.fn(),
      getLogDir: () => logDir,
      getLogFilePath: () => join(logDir, 'openx-current.log'),
      getRecentLogs: vi.fn(),
...
vi.mock('@electron/utils/openclaw-workspace', () => ({
  ensureOpenXContext: (...args: unknown[]) => ensureOpenXContextMock(...args),
}));
...
describe('host services', () => {
  beforeEach(() => {
    ...
    ensureOpenXContextMock.mockResolvedValue(undefined);
  ...
  it('accepts chat workspace settings through the typed settings API', async () => {
    ...
    await expect(api.set({ key: 'chatWorkspacePath', value: '/Users/alex/workspace/OpenX' })).resolves.toEqual({ success: true });
    await expect(api.set({ key: 'recentWorkspacePaths', value: ['/Users/alex/workspace/OpenX'] })).resolves.toEqual({ success: true });
    expect(setSettingMock).toHaveBeenCalledWith('chatWorkspacePath', '/Users/alex/workspace/OpenX');
    expect(setSettingMock).toHaveBeenCalledWith('recentWorkspacePaths', ['/Users/alex/workspace/OpenX']);
  });
  ...
  it('returns diagnostics snapshot with channel view and log tails', async () => {
    ...
    readLogFileMock.mockResolvedValue('openx-log-tail');
  ...
  it('reads only selected log files from the log directory', async () => {
    const selectedLog = join(logDir, 'openx-selected.log');
    writeFileSync(selectedLog, 'one\ntwo\nthree\n');
    listLogFilesMock.mockResolvedValue([{ name: 'openx-selected.log', path: selectedLog, size: 14, modified: 'now' }]);
