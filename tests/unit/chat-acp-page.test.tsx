...
describe('ACP Chat page', () => {
  ...
  it('uses OpenClaw session workspacePath for ACP load and read-only footer', async () => {
    chatState.sessions = [{ key: 'agent:main:session-a', workspacePath: '/Users/alex/workspace/OpenX' }];
    ...
    await waitFor(() => {
      expect(acpState.loadSession).toHaveBeenCalledWith({
        ...
        workspaceRoot: '/Users/alex/workspace/OpenX',
        cwd: '/Users/alex/workspace/OpenX',
      });
    });
    expect(screen.getByTestId('mock-workspace-path')).toHaveTextContent('/Users/alex/workspace/OpenX');
  ...
  it('reloads the same ACP session when summary hydration replaces fallback cwd', async () => {
    ...
    chatState.sessions = [{ key: sessionKey, updatedAt: 1000, workspacePath: '/Users/alex/workspace/OpenX' }];
    rerender(<Chat />);
  ...
  it('keeps a remote Gateway workspace usable when that path is missing on the desktop', async () => {
