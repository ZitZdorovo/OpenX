...
describe('ACP timeline reducer', () => {
  ...
  it('preserves resource link metadata and accepts OpenX staging ownership only from user content', () => {
    const resource = {
  ...
  it('renders a staged user image block as an attachment and ignores untrusted display paths', () => {
    expect(contentBlockToRenderPart({
      ...
      data: 'abc123',
      mimeType: 'image/png',
      uri: '/tmp/openx-staging/photo.png',
      _meta: {
        openx: {
    ...
    expect(contentBlockToRenderPart({
      type: 'resource_link',
      uri: '/tmp/openx-staging/notes.txt',
      name: 'notes.txt',
      _meta: { openx: { stagingId: 'stage-notes', displayPath: '/spoofed/private/notes.txt' } },
    }, {
