...
describe('patch-nsis-extract', () => {
  ...
  it('replaces CopyFiles-based extractUsing7za with direct 7z extraction', () => {
    tempDir = mkdtempSync(join(tmpdir(), 'openx-patch-nsis-'));
    ...
    expect(result).toContain('OpenX-patched-v2');
    ...
    expect(result).toContain('Restoring previous OpenX installation after failed update');
    expect(result).not.toContain('continuing overwrite install anyway');
    expect(patchNsisExtractTemplate(target)).toBe(true);
  });

  it('upgrades stale OpenX extract patches that used to continue after extract failure', () => {
    tempDir = mkdtempSync(join(tmpdir(), 'openx-patch-nsis-'));
    const target = join(tempDir, 'extractAppPackage.nsh');
    writeFileSync(
      target,
  ...
  ; OpenX-patched: extract directly to $INSTDIR.
  ClearErrors
  Nsis7z::Extract "\${FILE}"
...
!macroend`,
    ...
    const result = readFileSync(target, 'utf8');
    expect(result).toContain('OpenX-patched-v2');
    expect(result).toContain('Failed to extract OpenX files after multiple attempts.');
    expect(result).toContain('$(decompressionFailed)');
    expect(result).toContain('Quit');
    expect(result).toContain('SetErrorLevel 2');
    expect(result).toContain('Restoring previous OpenX installation after failed update');
