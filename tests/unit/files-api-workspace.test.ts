...
vi.mock('electron', () => ({
  app: { getPath: vi.fn(() => '/tmp/openx-user-data') },
...
describe('workspace-scoped files api', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = await mkdtemp(join(tmpdir(), 'openx-files-workspace-'));
    mocks.home = testDir;
  ...
  symlinkTest('registers staged file and directory ids with Main-owned source storage', async () => {
    ...
    expect(result.stagedPath).toContain(join('media', 'outbound', 'openx-staging'));
    expect(pathResult.stagedPath).toContain(join('media', 'outbound', 'openx-staging'));
