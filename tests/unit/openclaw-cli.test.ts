...
const mockedEntryPath = 'C:\\Program Files\\OpenX\\resources\\openclaw\\openclaw.mjs';
...
describe('getOpenClawCliCommand (Windows packaged)', () => {
  beforeEach(() => {
    ...
    setResourcesPath('C:\\Program Files\\OpenX\\resources');
  });
  ...
  it('falls back to ELECTRON_RUN_AS_NODE command when wrappers are missing', async () => {
    ...
    expect(command.endsWith("'C:\\Program Files\\OpenX\\resources\\openclaw\\openclaw.mjs'")).toBe(true);
  });
});
...
describe('getOpenClawCliSpawnSpec', () => {
  ...
  it('returns the packaged POSIX wrapper path as the spawn command', async () => {
    setPlatform('linux');
    mockIsPackagedGetter.value = true;
    setResourcesPath('/opt/OpenX/resources');
    mockExistsSync.mockImplementation((p: string) => p === '/opt/OpenX/resources/cli/openclaw');

    const { getOpenClawCliSpawnSpec } = await import('@electron/utils/openclaw-cli');
    const spec = getOpenClawCliSpawnSpec();

    expect(spec).toEqual({ command: '/opt/OpenX/resources/cli/openclaw', args: [], shell: false });
  });
  ...
  it('uses ELECTRON_RUN_AS_NODE with process.execPath when packaged wrappers are missing', async () => {
    const execPath = '/Applications/OpenX.app/Contents/MacOS/OpenX';
    setPlatform('darwin');
    mockIsPackagedGetter.value = true;
    setResourcesPath('/Applications/OpenX.app/Contents/Resources');
...
describe('getOpenClawEmbeddedForkSpec', () => {
  ...
  it('uses the packaged macOS Helper executable instead of the visible app executable', async () => {
    const execPath = '/Applications/OpenX.app/Contents/MacOS/OpenX';
    const helperPath = '/Applications/OpenX.app/Contents/Frameworks/OpenX Helper.app/Contents/MacOS/OpenX Helper';
    setPlatform('darwin');
    mockIsPackagedGetter.value = true;
    setResourcesPath('/Applications/OpenX.app/Contents/Resources');
    setExecPath(execPath);
  ...
  it('uses a real Node executable from PATH for dev embedded launches instead of Electron', async () => {
    const execPath = '/Users/zhuoxu/workspace/OpenX/node_modules/electron/dist/Electron.app/Contents/MacOS/Electron';
    setPlatform('darwin');
    setExecPath(execPath);
  ...
  it('fails packaged macOS embedded launch when the Helper executable is missing', async () => {
    ...
    expect(() => getOpenClawEmbeddedForkSpec(['acp'])).toThrow('OpenX Helper executable not found');
  });
});
