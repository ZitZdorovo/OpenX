...
describe('hostApi facade', () => {
  ...
  it('passes log file path and tail lines through hostInvoke', async () => {
    ...
    await expect(hostApi.logs.readFile('/tmp/openx.log', 50)).resolves.toEqual({ content: 'tail' });
    expect(hostInvoke).toHaveBeenCalledWith(expect.objectContaining({
      module: 'logs',
      action: 'readFile',
      payload: { path: '/tmp/openx.log', tailLines: 50 },
    }));
  });
  ...
  it('calls OmniRoute provider limits through the typed usage facade', async () => {
    hostInvoke.mockResolvedValueOnce({
      id: 'req',
      ok: true,
    ...
    await hostApi.usage.providerLimits({
      providerKey: 'custom-omniroute',
      forceRefresh: true,
    });
    expect(hostInvoke).toHaveBeenCalledWith(expect.objectContaining({
      module: 'usage',
      action: 'providerLimits',
      payload: { providerKey: 'custom-omniroute', forceRefresh: true },
    }));
  });
