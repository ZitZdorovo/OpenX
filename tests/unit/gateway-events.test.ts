...
describe('gateway store event wiring', () => {
  ...
  it('loads every remote OpenClaw session page', async () => {
    hostApiMock.gateway.rpc.mockImplementation((method: string, params?: { offset?: number }) => {
      if (method !== 'sessions.list') return Promise.resolve({});
      if ((params?.offset ?? 0) === 0) {
        return Promise.resolve({
          ts: 10,
          sessions: [{ key: 'agent:main:first', updatedAt: 10 }],
          hasMore: true,
          nextOffset: 1,
        });
      }
      return Promise.resolve({
        ts: 11,
        sessions: [{ key: 'agent:main:second', updatedAt: 11 }],
        hasMore: false,
      });
    });

    const { useChatStore } = await import('@/stores/chat');
    useChatStore.setState({ sessions: [], currentSessionKey: 'agent:main:first' });
    ...
    expect(hostApiMock.gateway.rpc).toHaveBeenCalledWith(
      'sessions.list',
      expect.objectContaining({ offset: 1, includeGlobal: true, includeUnknown: true }),
      undefined,
    );
  });
