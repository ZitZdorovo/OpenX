...
const testOpenClawDir = join(tmpdir(), `openx-session-workspace-${process.pid}`);
const testOpenClawConfigDir = join(tmpdir(), `openx-session-config-${process.pid}`);
...
describe('sessions API workspace summaries', () => {
  ...
  it('returns OpenClaw ACP cwd as workspacePath when available', async () => {
    seedAcpCwd('agent:main:session-a', '/Users/alex/workspace/OpenX');
    const { createSessionsApi } = await import('@electron/services/sessions-api');
