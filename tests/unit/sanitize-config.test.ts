...
function withOpenXToolDefaults<T extends Record<string, unknown>>(config: T): T & {
...
async function sanitizeConfig(
  ...
  if (plugins && typeof plugins === 'object' && !Array.isArray(plugins)) {
    ...
    // the provider is actually active, while OpenX-critical bundled plugins are
  ...
  // Mirror: OpenX keeps Skill Workshop disabled even when OpenClaw exposes it
...
beforeEach(async () => {
  tempDir = await mkdtemp(join(tmpdir(), 'openx-test-'));
...
describe('sanitizeOpenClawConfig (blocklist approach)', () => {
  it('removes skills.enabled at the root level of skills', async () => {
    ...
    expect(result.gateway).toEqual(withOpenXToolDefaults({ gateway: { mode: 'local' } }).gateway);
  });
  ...
  it('does nothing when config is already valid', async () => {
    const original = withOpenXToolDefaults({
      skills: {
  ...
  it('handles config with no skills section', async () => {
    ...
    const result = await readConfig();
    expect(result).toEqual(withOpenXToolDefaults(original));
  });
  ...
  it('handles empty config', async () => {
    ...
    const result = await readConfig();
    expect(result).toEqual(withOpenXToolDefaults({}));
  });
  ...
  it('preserves all other top-level config sections', async () => {
    ...
    expect(result.gateway).toEqual(withOpenXToolDefaults({
      gateway: {
        mode: 'local',
        auth: { token: 'xyz' },
      },
  ...
  it('sets session.dmScope to per-channel-peer when unset', async () => {
    await writeConfig(withOpenXToolDefaults({}));
  ...
  it('overrides session.dmScope when set to main', async () => {
    ...
    const base = withOpenXToolDefaults({});
    await writeConfig({
      ...base,
