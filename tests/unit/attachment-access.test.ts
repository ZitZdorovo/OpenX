...
describe('attachment access boundary', () => {
  ...
  beforeEach(async () => {
    clearAcpTraceForTests();
    testDir = await mkdtemp(join(tmpdir(), 'openx-attachment-access-'));
    ...
    registry.commitGrant({
      sessionKey,
      generation: 1,
      workspaceRoot: await realpath(workspaceRoot),
      executionCwd: await realpath(join(workspaceRoot, 'nested')),
    });
  ...
  it('requires a Main-owned staging id and matching staged path', async () => {
    const stagingDir = join(stateDir, 'media', 'outbound', 'openx-staging');
  ...
  it('does not treat a remote Gateway cwd as authority over desktop files', async () => {
    const stagedPath = join(outsideDir, 'uploaded.txt');
    await writeFile(stagedPath, 'uploaded');
    stagedAttachments.register('remote-upload', await realpath(stagedPath), 'uploaded.txt');
    registry.commitGrant({
      sessionKey,
      generation: 2,
    ...
    await expect(access.resolveAttachment({
      ref: { sessionKey, generation: 2, uri: stagedPath },
    })).resolves.toMatchObject({ ok: false, error: 'unavailable' });
    await expect(access.resolveAttachment({
      ref: { sessionKey, generation: 2, uri: stagedPath, stagingId: 'remote-upload' },
