...
describe('ACP Chat page inline timeline lifecycle', () => {
  ...
  it('renders ACP process blocks in timeline order', async () => {
    ...
    const planTrigger = screen.getByTestId('acp-plan-progress-trigger');
    expect(planTrigger).toHaveAttribute('aria-expanded', 'false');
    fireEvent.click(planTrigger);
    expect(screen.getByTestId('acp-plan-progress-popover')).toHaveTextContent('Update Chat page tests');
