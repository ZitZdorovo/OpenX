...
const geminiLowModelRef = 'custom-alpha123/agy/gemini-3.1-pro-low';
const geminiMaxModelRef = 'custom-alpha123/agy/gemini-pro-agent';

test.describe('OpenX chat model picker', () => {
  test('stores model and thinking on the remote chat session without restarting Gateway', async ({ launchElectronApp }) => {
    ...
    try {
      await app.evaluate(async ({ app: _app }, refs) => {
        ...
        const workspacePath = '/tmp/openx-model-picker-workspace';
      ...
      await expect(page.getByTestId('chat-thinking-picker-button')).toContainText('Off');
      await page.getByTestId('chat-thinking-picker-button').click();
      await page.getByTestId('chat-thinking-picker-menu').getByRole('button', { name: 'High' }).click();
      ...
      await expect(page.getByTestId('chat-model-picker-menu')).toContainText('Model Beta');
      ...
      await page.getByRole('button', { name: 'Edit name for Model Beta' }).click();
      await page.getByLabel('Edit model name').fill('Beta Research');
      await page.getByTitle('Save model name').click();
      await expect(page.getByTestId('chat-model-picker-menu')).toContainText('Beta Research');
      await page.getByTestId('chat-model-picker-option-model-beta').click();
      await expect(page.getByTestId('chat-model-picker-button')).toContainText('Beta Research');

      await page.getByTestId('chat-model-picker-button').click();
      await expect(page.getByTestId('chat-model-picker-menu')).toContainText('Gemini Pro Agent');
      await page.getByTestId('chat-model-picker-option-gemini-pro-agent').click();
      await expect(page.getByTestId('chat-thinking-picker-button')).toContainText('Low');
      await page.getByTestId('chat-thinking-picker-button').click();
      await page.getByTestId('chat-thinking-picker-menu').getByRole('button', { name: 'Max' }).click();
      await expect(page.getByTestId('chat-thinking-picker-button')).toContainText('Max');
      ...
      expect(requests.some((request) => (
        ...
        && (request.body as { model?: string; thinkingLevel?: string })?.model === betaModelRef
        && (request.body as { thinkingLevel?: string })?.thinkingLevel === 'low'
      ))).toBe(false);
      expect(requests).toContainEqual({
        path: 'gateway:sessions.patch',
        method: 'RPC',
        body: { key: 'agent:main:main', model: geminiLowModelRef, thinkingLevel: null },
      });
      expect(requests).toContainEqual({
        path: 'gateway:sessions.patch',
        method: 'RPC',
        body: { key: 'agent:main:main', model: geminiMaxModelRef, thinkingLevel: null },
      });
      expect(requests.some((request) => (
        request.path === 'gateway:sessions.patch'
        && ['low', 'max'].includes((request.body as { thinkingLevel?: string })?.thinkingLevel ?? '')
      ))).toBe(false);
      expect(requests.some((request) => request.path === 'agents:updateModel')).toBe(false);
      expect(requests.some((request) =>
