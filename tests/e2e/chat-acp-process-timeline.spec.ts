...
test.describe('OpenX ACP chat timeline', () => {
  test('renders inline ACP thought, tool, permission, and plan blocks from mocked IPC', async ({ launchElectronApp }) => {
    ...
    try {
      ...
      await emitAcpSessionUpdates(app, [
        {
          sessionUpdate: 'plan',
          entries: [
            { content: 'Inspect the existing chat flow', status: 'completed' },
            { content: 'Update Chat page tests', status: 'in_progress' },
            { content: 'Run the verification suite', status: 'pending' },
          ],
        },
        {
      ...
      const planTrigger = page.getByTestId('acp-plan-progress-trigger');
      await expect(planTrigger).toContainText('Step 2 / 3');
      await planTrigger.click();
      await expect(page.getByTestId('acp-plan-progress-popover')).toContainText('Inspect the existing chat flow');
      await expect(page.getByTestId('acp-plan-progress-popover')).toContainText('Update Chat page tests');
      await expect(page.getByTestId('acp-plan-progress-popover')).toContainText('Run the verification suite');
