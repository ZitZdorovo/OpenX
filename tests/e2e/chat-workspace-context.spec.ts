...
const SESSION_WORKSPACE = '/Users/e2e/workspace/OpenX';
const SESSION_WORKSPACE_LABEL = 'OpenX';
...
async function installWorkspaceMocks(app: ElectronApplication, options: WorkspaceMockOptions = {}) {
  ...
  const pinnedOrganizationSnapshot = {
    version: 1,
    projects: [],
    folders: [],
    placements: [],
    workspacePaths: {},
    pinnedChatKeys: [SESSION_KEY],
    pinnedProjectIds: [],
    pinnedFolderIds: [],
    collapsedProjectIds: [],
    collapsedFolderIds: [],
  ...
  await installIpcMocks(app, {
    ...
    hostApi: {
      ...
      [stableStringify(['sessions', 'rename', { id: SESSION_KEY, title: 'Header rename' }])]: { success: true },
      [stableStringify(['sessions', 'rename', { id: SESSION_KEY, title: 'Renamed conversation' }])]: { success: true },
      [stableStringify(['chatOrganization', 'pinChat', { chatKey: SESSION_KEY, pinned: true }])]: pinnedOrganizationSnapshot,
      [stableStringify(['chatOrganization', 'pinChat', { beforeChatKey: undefined, chatKey: SESSION_KEY, pinned: true }])]: pinnedOrganizationSnapshot,
    },
...
test.describe('OpenX chat workspace context', () => {
  test('bound session shows read-only workspace and workspace tree uses the same cwd', async ({ launchElectronApp }) => {
    ...
    try {
      ...
      await expect(workspaceSelector).toContainText('OpenX');
  ...
  test('chat header shows the current sidebar title and follows session renames', async ({ launchElectronApp }) => {
    ...
    try {
      ...
      const titleInput = page.getByTestId('chat-session-title-input');
      await expect(titleInput).toBeVisible();
      await expect(titleInput).toHaveValue('Workspace chat');
      await expect(titleInput).toHaveCSS('box-shadow', /inset/);
      await titleInput.fill('Header rename');
      await titleInput.press('Enter');
      await expect(page.getByTestId('chat-session-title')).toHaveText('Header rename');

      await page.getByTestId('chat-title-menu-button').click();
      await expect(page.getByTestId('chat-title-menu')).toBeVisible();
      await expect(page.getByTestId('chat-title-menu')).toContainText('Pin chat');
      await page.getByTestId('chat-title-menu').getByRole('button', { name: 'Rename chat', exact: true }).click();
      const dialogTitleInput = page.getByTestId('chat-session-title-dialog-input');
      await expect(dialogTitleInput).toBeVisible();
      await expect(dialogTitleInput).toHaveCSS('box-shadow', /inset/);
      await page.keyboard.press('Escape');
      await page.getByTestId('chat-title-menu-button').click();
      await page.getByTestId('chat-title-menu').getByRole('button', { name: 'Pin chat', exact: true }).click();

      const sidebarSession = page.getByTestId(`sidebar-session-${SESSION_KEY}`);
      await expect(page.getByTestId('sidebar-section-pinned').getByTestId(`sidebar-session-${SESSION_KEY}`)).toBeVisible();
      await expect(sidebarSession).toHaveAttribute('draggable', 'true');
      const sidebarDivider = page.getByTestId('sidebar-resize-handle').locator('span');
      const [sidebarSurfaceBox, sidebarDividerBox] = await Promise.all([
        page.getByTestId('sidebar').boundingBox(),
        sidebarDivider.boundingBox(),
      ]);
      expect(sidebarSurfaceBox).not.toBeNull();
      expect(sidebarDividerBox).not.toBeNull();
      expect(Math.round(sidebarDividerBox!.width)).toBe(1);
      expect(sidebarDividerBox!.x).toBeGreaterThanOrEqual(sidebarSurfaceBox!.x);
      expect(sidebarDividerBox!.x + sidebarDividerBox!.width).toBeLessThanOrEqual(sidebarSurfaceBox!.x + sidebarSurfaceBox!.width);
      expect(sidebarDividerBox!.y).toBeGreaterThan(sidebarSurfaceBox!.y);
      expect(sidebarDividerBox!.y + sidebarDividerBox!.height).toBeLessThan(sidebarSurfaceBox!.y + sidebarSurfaceBox!.height);
      await expect(sidebarDivider).toHaveCSS('mask-image', /linear-gradient/);
      await expect(page.getByTestId('chat-page')).toHaveCSS('border-left-width', '0px');
      const activeStyle = await sidebarSession.evaluate((element) => {
        const style = getComputedStyle(element);
        return { backgroundImage: style.backgroundImage, backgroundColor: style.backgroundColor, borderRadius: style.borderRadius };
      });
      expect(activeStyle.backgroundImage).toBe('none');
      ...
      const contextMenu = page.getByTestId('chat-context-menu');
      await expect(contextMenu).toBeVisible();
      const [sidebarBox, contextMenuBox] = await Promise.all([
        page.getByTestId('sidebar').boundingBox(),
        contextMenu.boundingBox(),
      ]);
      expect(sidebarBox).not.toBeNull();
      expect(contextMenuBox).not.toBeNull();
      expect(contextMenuBox!.x + contextMenuBox!.width).toBeGreaterThan(sidebarBox!.x + sidebarBox!.width);
      await contextMenu.getByRole('button', { name: 'Rename', exact: true }).click();
      const sidebarTitleInput = page.getByTestId('sidebar-chat-rename-input');
      await expect(sidebarTitleInput).toBeVisible();
      await sidebarTitleInput.fill('Renamed conversation');
      await sidebarTitleInput.press('Enter');
  ...
  test('remote inherited workspace opens even when its path is missing on this desktop', async ({ launchElectronApp }) => {
    const app = await launchElectronApp({ skipSetup: true });
