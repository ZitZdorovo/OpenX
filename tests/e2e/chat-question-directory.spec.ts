...
test.describe('OpenX chat question directory', () => {
  test('opens the ACP question directory for seeded history', async ({ launchElectronApp }) => {
    ...
    try {
      ...
      await emitAcpSessionUpdates(app, [
        ...messagesToAcpUpdates(seededHistory),
        { sessionUpdate: 'usage_update', used: 13, size: 100 },
      ]);
      await expect(page.getByTestId('acp-chat-timeline')).toBeVisible({ timeout: 30_000 });
      await expect(page.getByTestId('acp-chat-timeline')).toHaveCSS('user-select', 'text');
      await expect(page.getByTestId('chat-composer-input')).toHaveCSS('user-select', 'text');
      const scrollNavigator = page.getByTestId('chat-scroll-navigator');
      await expect(scrollNavigator).toBeVisible();
      const [composerBox, composerSurfaceBox, composerInputBox, composerSendBox, workspaceSelectorBox, requestStatsBox] = await Promise.all([
        page.getByTestId('chat-composer').boundingBox(),
        page.getByTestId('chat-composer-surface').boundingBox(),
        page.getByTestId('chat-composer-input').boundingBox(),
        page.getByTestId('chat-composer-send').boundingBox(),
        page.getByTestId('chat-workspace-selector').boundingBox(),
        page.getByTestId('chat-request-stats-button').boundingBox(),
      ]);
      ...
      await expect(firstScrollMarker.locator('span')).toHaveCSS('height', '2px');
      const [firstMarkerBox, secondMarkerBox] = await Promise.all([
        firstScrollMarker.locator('span').boundingBox(),
        secondScrollMarker.locator('span').boundingBox(),
      ]);
      expect(firstMarkerBox).not.toBeNull();
      expect(secondMarkerBox).not.toBeNull();
      expect(Math.abs(
        (firstMarkerBox!.x + firstMarkerBox!.width / 2)
        - (secondMarkerBox!.x + secondMarkerBox!.width / 2),
      )).toBeLessThanOrEqual(1);
      await firstScrollMarker.hover();
      await expect(firstScrollMarker.locator('span')).toHaveCSS('width', '32px');
      const scrollPreview = page.getByTestId('chat-scroll-preview');
      await expect(scrollPreview).toContainText('First question: summarize the market opening.');
      await expect(scrollPreview).toContainText('This answer intentionally contains enough text');
