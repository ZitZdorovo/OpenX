  ...
  test('renders DOCX and isolated PPTX decks through real Electron Host APIs', async ({ launchElectronApp }) => {
    ...
    try {
      ...
      await expect.poll(async () => docxHost.evaluate((host) => {
        ...
        return {
          pageCount: root?.querySelectorAll('section.openx-docx').length ?? 0,
      ...
      expect(renderedDocxText).toContain('OpenX Office Preview Header');
      ...
      expect(renderedDocxText).toContain('OpenX Office Preview Footer');
