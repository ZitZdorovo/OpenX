...
test.describe('OpenX ACP inline timeline', () => {
  ...
  test('starts on a new empty chat instead of selecting a heartbeat-only OpenX session', async ({ launchElectronApp }) => {
  ...
  test('hides heartbeat-only OpenX sessions from the sidebar without hiding normal sessions', async ({ launchElectronApp }) => {
    const app = await launchElectronApp({ skipSetup: true });
