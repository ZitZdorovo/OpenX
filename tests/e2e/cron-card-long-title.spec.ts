...
test.describe('Cron job card layout', () => {
  test('keeps the toggle switch fully inside the card when the title is very long', async ({ launchElectronApp }) => {
    ...
    const refreshButton = page.getByTestId('cron-refresh-button');
