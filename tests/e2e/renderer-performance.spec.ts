...
async function startFrameSampling(page: Page, durationMs: number): Promise<void> {
  await page.evaluate((duration) => {
    const perfWindow = window as typeof window & { __openxPerfFrameSamples?: number[] };
    ...
    perfWindow.__openxPerfFrameSamples = samples;
...
async function readFramePacing(page: Page): Promise<FramePacing> {
  const samples = await page.evaluate(() => (
    (window as typeof window & { __openxPerfFrameSamples?: number[] }).__openxPerfFrameSamples ?? []
  ));
...
test('profiles a populated timeline during a growing Markdown stream', async ({ launchElectronApp }, testInfo) => {
  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      Object.assign(window, { __openxPerfLongTasks: longTasks, __openxPerfObserver: observer });
    });
    ...
    const longTasks = await page.evaluate(() => {
      const perfWindow = window as typeof window & {
        __openxPerfLongTasks?: number[];
        __openxPerfObserver?: PerformanceObserver;
      };
      perfWindow.__openxPerfObserver?.disconnect();
      return perfWindow.__openxPerfLongTasks ?? [];
    });
    await cdp.detach();
    ...
    console.log(`OpenX chat performance: ${JSON.stringify(benchmark)}`);
...
test('profiles sidebar animation and scrolling with rich static Markdown', async ({ launchElectronApp }, testInfo) => {
  ...
  try {
    ...
    await expect(page.locator('.openx-streamdown [data-streamdown="code-block-body"] pre').first()).toBeAttached({ timeout: 30_000 });
    await waitForPaint(page);
    ...
    const dom = await page.evaluate(() => ({
      ...
      markdownNodes: document.querySelectorAll('.openx-streamdown *').length,
    }));
    await cdp.detach();
    ...
    console.log(`OpenX interaction performance: ${JSON.stringify(benchmark)}`);
