...
import { AlertTriangle, ArrowDownToLine, Copy, Folder, FolderOpen, MoreHorizontal, Pencil, Pin } from 'lucide-react';
...
import type { AcpTimelineSnapshot, MessageSegmentItem, PlanItem, RenderPart } from '@/lib/acp/timeline-types';
...
import { ChatScrollNavigator, type ChatScrollNavigatorItem } from './ChatScrollNavigator';
import { AcpPlanProgress } from './AcpPlanProgress';
...
function previewText(item: MessageSegmentItem | undefined, fallback = ''): string {
  const value = item?.parts
...
function buildChatScrollNavigatorItems(snapshot: AcpTimelineSnapshot): ChatScrollNavigatorItem[] {
  const ordered = snapshot.itemOrder.map((itemId) => snapshot.itemsById[itemId]);
  const result: ChatScrollNavigatorItem[] = [];
  for (let index = 0; index < ordered.length; index += 1) {
    const item = ordered[index];
    if (item?.kind !== 'message-segment' || item.role !== 'user') continue;
    let assistant: MessageSegmentItem | undefined;
    for (let cursor = index + 1; cursor < ordered.length; cursor += 1) {
      const candidate = ordered[cursor];
      if (candidate?.kind !== 'message-segment') continue;
    ...
    result.push({
      ...
      userPreview: previewText(item, 'Message'),
      assistantPreview: assistant ? previewText(assistant) : undefined,
    });
  }
  return result;
}
...
export function Chat() {
  ...
  const organizedWorkspacePath = useChatOrganizationStore((state) => (
    currentSessionKey ? state.workspacePaths[currentSessionKey] : undefined
  ));
  const pinnedChatKeys = useChatOrganizationStore((state) => state.pinnedChatKeys);
  const pinChat = useChatOrganizationStore((state) => state.pinChat);
  ...
  const commitSessionTitle = useCallback(async () => {
    if ((!editingSessionTitle && !inlineEditingSessionTitle) || !currentSessionKey) return;
    const nextTitle = sessionTitleDraft.trim();
    setEditingSessionTitle(false);
    setInlineEditingSessionTitle(false);
    if (!nextTitle || nextTitle === currentSessionTitle) return;
    try {
      await renameSession(currentSessionKey, nextTitle);
    } catch (error) {
      toast.error(t('titleRenameFailed', { error: String(error) }));
    }
  }, [currentSessionKey, currentSessionTitle, editingSessionTitle, inlineEditingSessionTitle, renameSession, sessionTitleDraft, t]);
  const currentChatPinned = currentSessionKey ? pinnedChatKeys.includes(currentSessionKey) : false;

  useEffect(() => {
    if (!titleMenuOpen) return;
    const closeTitleMenu = (event: PointerEvent) => {
      if (!titleMenuRef.current?.contains(event.target as Node)) setTitleMenuOpen(false);
    };
    window.addEventListener('pointerdown', closeTitleMenu);
    return () => window.removeEventListener('pointerdown', closeTitleMenu);
  }, [titleMenuOpen]);
  const effectiveWorkspace = useMemo(() => resolveEffectiveWorkspace({
    session: currentSession && organizedWorkspacePath
      ? { ...currentSession, workspacePath: organizedWorkspacePath }
      : currentSession,
    globalWorkspace: chatWorkspacePath,
  }), [chatWorkspacePath, currentSession, organizedWorkspacePath]);
  ...
  const activePlanItem = useMemo<PlanItem | null>(() => {
  ...
  const attachChatScrollElement = useCallback((element: HTMLElement | null) => {
    scrollRef(element);
    setChatScrollElement((current) => current === element ? current : element);
  }, [scrollRef]);
  ...
  const scrollNavigatorItems = useMemo(
    () => buildChatScrollNavigatorItems(visibleAcpTimeline),
    [visibleAcpTimeline],
  );
  ...
  return (
    <div
      ...
      className={cn(
        ...
        'bg-surface-chat',
        isMac && 'z-20 rounded-tl-2xl border-t border-border/55',
        isWindows && 'rounded-tl-2xl border-t border-border/70',
      )}
    ...
    >
      <div className="flex min-w-0 flex-1 flex-col">
              ...
              {titleMenuOpen && (
                <div className="no-drag absolute left-5 top-full z-50 mt-1 w-64 rounded-xl border border-black/10 bg-surface-modal p-1.5 text-sm shadow-xl dark:border-white/10" data-testid="chat-title-menu">
                  ...
                  <button
                    ...
                    className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-foreground/85 hover:bg-black/5 hover:text-foreground dark:hover:bg-white/10"
                    onClick={() => {
                      void navigator.clipboard.writeText(currentSessionTitle || '');
        ...
        <div className="relative min-h-0 flex-1 overflow-hidden px-4 py-4">
          <div className="relative mx-auto flex h-full min-h-0 w-full max-w-7xl flex-col">
            <div data-testid="chat-scroll-column" className="relative flex min-h-0 min-w-0 flex-1 flex-col">
              {(workspaceUnavailable || visibleAcpError) && (
                <div className="shrink-0 min-[900px]:pl-14">
              ...
              <div ref={attachChatScrollElement} className="min-h-0 min-w-0 flex-1 overflow-y-auto min-[900px]:pl-14" data-testid="chat-scroll-container">
              ...
              <ChatScrollNavigator
                ...
                scrollElement={chatScrollElement}
                label={t('scrollNavigator.label')}
              />
        ...
        {activePlanItem && (
          <AcpPlanProgress key={currentSessionKey ?? activePlanItem.id} item={activePlanItem} />
        )}
        ...
        <ChatInput
          ...
          usage={visibleAcpTimeline.metadata.usage}
        />
      </div>
      ...
      <Dialog
        open={editingSessionTitle}
        onOpenChange={(open) => {
          setEditingSessionTitle(open);
          if (!open) setSessionTitleDraft(currentSessionTitle || '');
        }}
      >
        <DialogContent className="max-w-[420px] rounded-2xl border border-border bg-surface-modal p-5 shadow-2xl">
          <DialogTitle className="text-xl font-semibold tracking-tight">{t('renameDialog.title')}</DialogTitle>
          <DialogDescription className="mt-1 text-sm text-muted-foreground">{t('renameDialog.description')}</DialogDescription>
          <Input
            autoFocus
            data-testid="chat-session-title-dialog-input"
            value={sessionTitleDraft}
            ...
            onChange={(event) => setSessionTitleDraft(event.target.value)}
            onKeyDown={(event) => {
              if (event.key === 'Escape') setEditingSessionTitle(false);
              if (event.key === 'Enter') void commitSessionTitle();
            }}
          />
          <div className="mt-4 flex justify-end gap-2">
            <Button variant="outline" onClick={() => setEditingSessionTitle(false)}>{t('renameDialog.cancel')}</Button>
            <Button disabled={!sessionTitleDraft.trim()} onClick={() => void commitSessionTitle()}>{t('renameDialog.save')}</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
