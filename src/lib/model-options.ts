...
export interface GatewayModelCatalogEntry {
  ...
  contextWindow?: number;
  available?: boolean;
  cost?: ConfiguredModelOption['pricing'];
}
...
/** Convert the remote Gateway's authoritative model catalog to picker rows. */
export function buildGatewayModelOptions(models: GatewayModelCatalogEntry[]): ConfiguredModelOption[] {
  const deduped = new Map<string, ConfiguredModelOption>();
  for (const model of models) {
    const id = model.id?.trim();
    const provider = model.provider?.trim();
    if (!id || !provider || model.available === false) continue;
    const modelRef = id.startsWith(`${provider}/`) ? id : `${provider}/${id}`;
    if (deduped.has(modelRef)) continue;
    deduped.set(modelRef, {
      modelRef,
