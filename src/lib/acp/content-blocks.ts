...
function openxUserMetadata(block: ContentBlock, role: ContentBlockRenderContext['role']): {
  stagingId?: string;
  fileName?: string;
} {
  if (role !== 'user') return {};
  const meta = recordValue(block._meta);
  const openx = recordValue(meta?.openx);
  const stagingId = nonEmptyString(openx?.stagingId);
  const fileName = nonEmptyString(openx?.fileName);
  return {
    ...(stagingId ? { stagingId } : {}),
    ...(fileName ? { fileName } : {}),
  };
...
export function contentBlockToRenderPart(block: ContentBlock, context: ContentBlockRenderContext): RenderPart {
  switch (block.type) {
    ...
    case 'image': {
      ...
      const openx = openxUserMetadata(block, context.role);
