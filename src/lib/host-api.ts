import type {
  ...
  OmniRouteLimitsConfigurePayload,
...
export const hostApi = {
  ...
  shell: {
    ...
    openTerminal: (path: string) => invokeHost('shell', 'openTerminal', { path } satisfies ShellPathPayload),
  },
  webBrowser: {
  ...
  window: {
    ...
    showMenu: (menu: WindowMenuId, x: number, y: number) => (
      invokeHost('window', 'showMenu', { menu, x, y })
    ),
  ...
  gateway: {
    connection: () => invokeHost('gateway', 'connection'),
    configure: (url: string, authMode: 'token' | 'password', credential: string) => (
      invokeHost('gateway', 'configure', { url, authMode, credential })
    ),
  ...
  agents: {
    ...
    personality: (id: string) => invokeHost('agents', 'personality', { id }),
    updatePersonality: (id: string, content: string) => (
      invokeHost('agents', 'updatePersonality', { id, content })
    ),
  ...
  chatOrganization: {
    snapshot: () => invokeHost('chatOrganization', 'snapshot'),
    createProject: (name: string, path: string) => (
      invokeHost('chatOrganization', 'createProject', { name, path })
    ),
    createFolder: (projectId: string, name: string, parentId?: string | null) => (
      invokeHost('chatOrganization', 'createFolder', { projectId, name, parentId })
    ),
    rename: (kind: 'project' | 'folder', id: string, name: string) => (
      invokeHost('chatOrganization', 'rename', { kind, id, name })
    ),
    deleteProject: (id: string, confirmed = false) => (
      invokeHost('chatOrganization', 'deleteProject', { id, confirmed })
    ),
    deleteFolder: (id: string, confirmed = false) => (
      invokeHost('chatOrganization', 'deleteFolder', { id, confirmed })
    ),
    moveChat: (chatKey: string, projectId: string, folderId?: string | null, beforeChatKey?: string | null) => (
      invokeHost('chatOrganization', 'moveChat', { chatKey, projectId, folderId, beforeChatKey })
    ),
    moveFolder: (folderId: string, projectId: string, parentId?: string | null) => (
      invokeHost('chatOrganization', 'moveFolder', { folderId, projectId, parentId })
    ),
    unplaceChat: (chatKey: string, workspacePath: string) => invokeHost('chatOrganization', 'unplaceChat', { chatKey, workspacePath }),
    pinChat: (chatKey: string, pinned: boolean, beforeChatKey?: string | null) => (
      invokeHost('chatOrganization', 'pinChat', { chatKey, pinned, beforeChatKey })
    ),
    pinNode: (kind: 'project' | 'folder', id: string, pinned: boolean) => (
      invokeHost('chatOrganization', 'pinNode', { kind, id, pinned })
    ),
    setCollapsed: (kind: 'project' | 'folder' | 'pinned', collapsed: boolean, id?: string) => (
      invokeHost('chatOrganization', 'setCollapsed', { kind, collapsed, id })
    ),
  },
  chat: {
  ...
  usage: {
    ...
    providerLimits: (payload?: ProviderLimitsPayload) => (
      invokeHost('usage', 'providerLimits', payload)
    ),
    omniRouteConfig: () => invokeHost('usage', 'omniRouteConfig'),
    configureOmniRoute: (payload: OmniRouteLimitsConfigurePayload) => (
      invokeHost('usage', 'configureOmniRoute', payload)
    ),
  },
};
