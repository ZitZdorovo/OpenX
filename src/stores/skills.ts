...
function mergeGatewaySkills(localSkills: Skill[], gatewaySkills?: GatewaySkillStatus[]): Skill[] {
  ...
  for (const gatewaySkill of gatewaySkills) {
    ...
    // Every row returned by the connected Gateway belongs in OpenX. Previous
    // local-client filtering hid bundled and ClawHub skills during hydration.
...
export const useSkillsStore = create<SkillsState>((set, get) => ({
  ...
  fetchSkills: async () => {
    ...
    try {
      // The connected Gateway is the only source of truth. `local()` is kept
      // in the host contract for compatibility, but Main resolves it through
      // the remote skills.status RPC as well.
      const result = await hostApi.skills.status();
      set({ skills: mergeGatewaySkills([], result.skills), loading: false, error: null });
      return true;
    } catch (error) {
      console.error('Failed to fetch remote Gateway skills:', error);
