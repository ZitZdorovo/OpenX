...
function synchronizeGatewaySessionCatalog(status: GatewayStatus): void {
  ...
  void (async () => {
    ...
    const remoteFeatureHydration = Promise.allSettled([
      import('./cron').then(({ useCronStore }) => useCronStore.getState().fetchJobs()),
      import('./channels').then(({ useChannelsStore }) => useChannelsStore.getState().fetchChannels()),
      import('./skills').then(({ useSkillsStore }) => useSkillsStore.getState().fetchSkills()),
      import('./model-catalog').then(({ useModelCatalogStore }) => useModelCatalogStore.getState().refresh()),
    ]);
    try {
      ...
      try {
        ...
        if (!gatewayEventUnsubscribers) {
          ...
          unsubscribers.push(hostEvents.onGatewayStatus((payload) => {
            ...
            // Cron, Channels, Skills and the model catalog are hydrated by
            // synchronizeGatewaySessionCatalog once per remote connection.
