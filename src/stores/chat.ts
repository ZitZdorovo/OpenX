...
function toTopicSessionLabel(text: string, maxLength = 50): string {
  const automatic = toAutomaticSessionLabel(text, 240)
    .replace(/\s+/g, ' ')
    .replace(/^(?:так[,.!]?\s+смотри[,.!]?\s*|привет[,.!]?\s*|пожалуйста[,.!]?\s*|можешь(?:те)?\s+|мне\s+нужно\s+|can\s+you\s+|could\s+you\s+|please\s+)/i, '')
    .replace(/^(?:пожалуйста[,.!]?\s*|please\s+)/i, '')
    .replace(/^(?:сделай(?:те)?\s+|создай(?:те)?\s+|добавь(?:те)?\s+|build\s+|create\s+|add\s+)/i, '')
    .trim();
  if (!automatic) return '';
  const sentence = automatic.split(/(?<=[.!?。！？])\s+/u)[0]?.trim() || automatic;
...
function isHeartbeatOnlySummaryForSession(
  ...
  if (!session || !isOpenXDesktopSessionKey(session.key)) return false;
...
async function fetchChatSessionsList(): Promise<Record<string, unknown>> {
  ...
  // The Gateway intentionally bounds an omitted sessions.list limit. Walk the
  // remote catalog so OpenX never presents an empty/partial history merely
  // because the user has more sessions than one response page.
  for (let page = 0; page < 100; page += 1) {
    ...
    sessions.push(...rows);
    if (typeof response.ts === 'number' && Number.isFinite(response.ts)) {
      latestTs = Math.max(latestTs ?? -Infinity, response.ts);
    }
    const nextOffset = typeof response.nextOffset === 'number' && Number.isFinite(response.nextOffset)
      ? response.nextOffset
      : null;
    if (response.hasMore !== true || nextOffset === null || nextOffset <= offset) break;
    offset = nextOffset;
  }

  return {
    ...(latestTs !== undefined ? { ts: latestTs } : {}),
    sessions,
  };
}
...
export const useChatStore = create<ChatState>((set, get) => ({
        ...
        try {
          ...
          if (generation === sessionCatalogGeneration) {
            ...
            let sessionsWithCurrent = !visibleMergedSessions.some((session) => session.key === nextSessionKey)
              && nextSessionKey
              && isOpenXDesktopSessionKey(nextSessionKey)
              ? [
                  ...visibleMergedSessions,
  ...
  setSessionWorkspace: (key, workspacePath) => {
    ...
    markLocalSessionCatalogMutation();
    set((state) => ({
      sessions: ensureSessionEntry(state.sessions, key, { workspacePath: normalized }),
    }));
  },
  ...
  acknowledgeAcpSessionCreated: (key, workspacePath, initialPrompt) => {
    ...
    const initialLabel = key.endsWith(':main') ? '' : toTopicSessionLabel(initialPrompt || '');
    ...
    if (initialLabel) {
      void hostApi.sessions.rename(key, initialLabel).then((result) => {
        if (!result.success) return;
        const currentState = useChatStore.getState();
        const session = currentState.sessions.find((entry) => entry.key === key);
        if (session) {
          finishSessionLabelHydration(
            key,
            getSessionLabelHydrationVersion(session, currentState.sessionLastActivity),
            'backend-label',
          );
        }
        markLocalSessionCatalogMutation();
        set((state) => ({
          sessions: state.sessions.map((entry) => entry.key === key ? { ...entry, label: initialLabel } : entry),
          sessionLabels: { ...state.sessionLabels, [key]: initialLabel },
        }));
      }).catch(() => {
        // Keep the local automatic topic if the remote Gateway is temporarily unavailable.
      });
    }
  },
