...
function parseThinkingLevels(value: unknown): ChatSession['thinkingLevels'] | undefined {
...
function parseThinkingOptions(value: unknown): string[] | undefined {
  if (!Array.isArray(value)) return undefined;
...
export function normalizeGatewaySessionPatch(raw: Record<string, unknown>): NormalizedSessionPatch {
  ...
  if (hasOwn(raw, 'thinkingLevels')) {
    ...
    if (raw.thinkingLevels === null) {
      ...
      const levels = parseThinkingLevels(raw.thinkingLevels);
      if (levels !== undefined) values.thinkingLevels = levels;
    }
  }

  if (hasOwn(raw, 'thinkingOptions')) {
    present.add('thinkingOptions');
    if (raw.thinkingOptions === null) {
      cleared.add('thinkingOptions');
    } else {
      const options = parseThinkingOptions(raw.thinkingOptions);
      if (options !== undefined) values.thinkingOptions = options;
    }
  }
