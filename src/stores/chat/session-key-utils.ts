...
const NON_USER_SESSION_LABELS = new Set(['openx', 'main']);
...
export function isOpenXDesktopSessionKey(sessionKey: string): boolean {
 ...
 * added to a group, webhook ping). Hide those placeholder entries from OpenX
...
export function isOpenClawHeartbeatOnlySession(session: ChatSession): boolean {
  if (!isOpenXDesktopSessionKey(session.key)) return false;
