...
interface SettingsState {
  ...
  setGatewayConnection: (url: string, authMode: 'token' | 'password') => void;
  ...
  setAgentBadgeMode: (mode: AgentBadgeMode) => void;
  setAgentBadgeAlias: (agentId: string, alias: string) => void;
...
const defaultSettings = {
  ...
  workspaceLabels: {},
  agentBadgeMode: 'full' as AgentBadgeMode,
  agentBadgeAliases: {},
  setupComplete: false,
};

const clampSidebarWidth = (value: number) => Math.min(520, Math.max(240, Math.round(value)));
...
export const useSettingsStore = create<SettingsState>()(
  persist(
    (set) => ({
      ...
      setGatewayConnection: (gatewayUrl, gatewayAuthMode) => set({ gatewayUrl, gatewayAuthMode }),
      ...
      setAgentBadgeMode: (agentBadgeMode) => {
        set({ agentBadgeMode });
        void hostApi.settings.set('agentBadgeMode', agentBadgeMode).catch(() => { });
      },
      setAgentBadgeAlias: (agentId, alias) => {
        const normalizedAgentId = agentId.trim();
        if (!normalizedAgentId) return;
        set((state) => {
          const normalizedAlias = alias.trim();
          const agentBadgeAliases = { ...state.agentBadgeAliases };
          if (normalizedAlias) agentBadgeAliases[normalizedAgentId] = normalizedAlias;
          else delete agentBadgeAliases[normalizedAgentId];
          void hostApi.settings.set('agentBadgeAliases', agentBadgeAliases).catch(() => { });
          return { agentBadgeAliases };
        });
      },
