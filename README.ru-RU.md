...
<a href="#почему-openx">Почему OpenX</a> •
...
## Режим удалённого Gateway

OpenX — самостоятельный внешний клиент для уже запущенного OpenClaw Gateway. OpenX не запускает, не контролирует, не ремонтирует и не завершает процесс Gateway. При первом запуске укажите адрес `ws://` или `wss://` и выберите авторизацию по токену или паролю. Секрет шифруется через Electron `safeStorage`, не возвращается в renderer и не записывается в обычные настройки. Cron, Channels, Skills, сессии и модели автоматически синхронизируются после подключения.

Чаты организованы локально как **Проекты → вложенные папки → чаты**, а закреплённые вынесены в отдельную сворачиваемую секцию. Cron, Channels, Skills, сессии и конфигурация работают только через подключённый удалённый Gateway. Встроенная команда OpenClaw сохранена исключительно для ACP stdio→WebSocket моста, а не для локального Gateway.

Пути рабочих каталогов сессий относятся к машине Gateway и не обязаны существовать на компьютере с OpenX. Выбранные на клиенте файлы встраиваются в ACP-запрос, поэтому удалённый агент получает содержимое файла, а не недоступный ему локальный staging-путь.

После подключения OpenX также регистрирует второй аутентифицированный WebSocket `node-host` с той же идентификацией устройства. Агенты OpenClaw видят клиент как OpenClaw Node и могут вызывать объявленные команды `openx.*`: фокус/навигацию окна и те же обработчики проектов, папок, переименования, перемещения и закрепления, которые использует UI. В `gateway.nodes.allowCommands` автоматически добавляются только эти объявленные команды; произвольный локальный shell клиент не открывает.

---

## Обзор

**OpenX** — удалённый десктоп-клиент для уже работающего [OpenClaw](https://github.com/OpenClaw).

Автоматизация рабочих процессов, управление AI-каналами или планирование интеллектуальных задач — OpenX предоставляет интерфейс для эффективного использования AI-агентов.

OpenX поставляется с предустановленными лучшими практиками для провайдеров моделей и нативно поддерживает Windows, а также многоязычные настройки. Вы можете тонко настроить расширенные параметры через **Настройки → Дополнительно → Режим разработчика**.
...
Создание AI-агентов не должно требовать владения командной строкой. Философия OpenX проста: **мощные технологии заслуживают интерфейса, который уважает ваше время.**

| Проблема | Решение OpenX |
|----------|---------------|
...
| Удалённое подключение | Клиент не владеет жизненным циклом Gateway |
...
OpenX построен непосредственно на официальном ядре **OpenClaw**. Вместо отдельной установки мы встраиваем среду выполнения в приложение для бесшовного опыта "всё включено".
...
При выборе другого агента через `@agent` OpenX переключается непосредственно в контекст этого агента вместо ретрансляции через агента по умолчанию. Рабочие пространства агентов по умолчанию разделены, но более строгая изоляция зависит от настроек песочницы OpenClaw.
...
На правой панели Chat остаются только вкладки «Рабочая область», «Просмотр» и «Изменения»; универсального веб-браузера, домашней страницы и адресной строки больше нет. Разрешённые локальные вложения `.html` / `.htm`, файловые операции и файлы рабочей области по умолчанию открываются в «Просмотре». В действиях файла можно выбрать встроенный просмотр OpenX или системное приложение, а кнопка в заголовке просмотра открывает текущий HTML-файл в системном браузере.

Все ссылки некликабельны. Ссылки, отображаемые OpenX, выглядят как обычный текст; в HTML-просмотре также удаляются оформление ссылок и взаимодействие указателем. Формы, переходы из скриптов, перенаправления, переходы внутри страницы, всплывающие окна, загрузки, сетевые запросы и разрешения устройств блокируются. Самодостаточный локальный HTML отображается, но не может покинуть выбранный документ.
...
Для пользовательских идентификаторов учётных записей каналов OpenX требует совместимый с OpenClaw канонический формат (`[a-z0-9_-]`, строчные буквы, максимум 64 символа, должен начинаться с буквы или цифры) для предотвращения ошибок маршрутизации.
OpenX также включает официальный плагин личного WeChat от Tencent, позволяя подключить WeChat напрямую со страницы Каналы через встроенный QR-код.
...
Страница «Навыки» загружает, включает, отключает и настраивает навыки исключительно через подключённый OpenClaw Gateway. Без соединения она показывает недоступное состояние; OpenX не поддерживает отдельный локальный набор навыков.
...
Когда совместимый шлюз отклоняет `/models` по причинам, не связанным с аутентификацией, OpenX автоматически переключается на легковесный зонд `/chat/completions` или `/responses` при проверке API-ключа.

### 🌙 Адаптивные темы
Светлая тема, тёмная тема или синхронизация с системой. OpenX автоматически адаптируется к вашим предпочтениям.

### 🚀 Управление автозапуском
В **Настройки → Общие** вы можете включить **Запускать при старте системы**, чтобы OpenX автоматически запускался после входа в систему.
...
Скачайте последний релиз для вашей платформы со страницы [Releases](https://github.com/ValueCell-ai/OpenX/releases).
...
При первом запуске OpenX **Мастер настройки** проведёт вас через:
...
OpenX включает встроенные настройки прокси для сред, где Electron, шлюз OpenClaw или каналы вроде Telegram должны выходить в интернет через локальный прокси-клиент.
...
- Если расширенные поля прокси пусты, OpenX использует `Прокси-сервер`.
- Сохранение настроек прокси повторно применяет сеть Electron и переподключает клиент, не перезапуская удалённый Gateway.
- OpenX также синхронизирует прокси с конфигурацией канала Telegram в OpenClaw, когда Telegram включён.
- Существующий прокси Telegram сохраняется, пока пользователь явно не изменит его.
...
OpenX использует **двухпроцессную архитектуру с унифицированным уровнем Host API**. Рендерер обращается к единой абстракции клиента, а Electron Main управляет выбором протокола и жизненным циклом процессов:

```
┌─────────────────────────────────────────────────────────────────┐
│                     Десктоп-приложение OpenX                    │
│                                                                 │
│  ┌────────────────────────────────────────────────────────────┐ │
│  │               Главный процесс Electron                     │ │
│  │  • Управление жизненным циклом окна и приложения           │ │
│  │  • Удалённый Gateway, RPC и автопереподключение            │ │
│  │  • Интеграция с системой и OS keychain                     │ │
...
### Модель процессов и диагностика подключения

- OpenX — это приложение Electron, поэтому **один экземпляр приложения обычно отображается как несколько процессов ОС** (main/renderer/zygote/utility). Это нормально.
...
- Нажатие кнопки закрытия окна (`X`) скрывает OpenX в трей; это **не** полностью закрывает приложение. Используйте меню трея **Quit OpenX** для полного завершения.
...
│   ├── gateway/              # Удалённое подключение и RPC OpenClaw
...
- использует временный каталог `userData` OpenX
...
Мы приветствуем вклад сообщества! Исправления багов, новые функции, улучшения документации или переводы — каждый вклад делает OpenX лучше.
...
OpenX построен на плечах отличных проектов с открытым исходным кодом:
...
### Партнёрская программа OpenX 🚀

Мы запускаем Партнёрскую программу OpenX и ищем партнёров, которые могут помочь представить OpenX большему числу клиентов, особенно тем, у кого есть потребности в кастомных AI-агентах или автоматизации.

Партнёры помогают связывать нас с потенциальными пользователями и проектами, а команда OpenX предоставляет полную техническую поддержку, кастомизацию и интеграцию.
...
OpenX выпускается под [лицензией MIT](LICENSE). Вы можете свободно использовать, модифицировать и распространять это программное обеспечение.
